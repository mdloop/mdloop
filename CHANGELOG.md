...
- `vorlyn serve start|stop|status`: a detached, persistent local instance (survives closing the
  terminal that started it), so multiple folders on one machine share one embedded Postgres +
  server instead of each `vorlyn open` fighting over the same ports. `vorlyn open` attaches to
...
- `vorlyn link`/`vorlyn open` auto-provision a project named after the folder (reusing one that
  already exists by name, or via a persisted folder→project mapping) instead of sharing one
  machine-wide default project — `vorlyn projects list` shows the mapping. An explicit `--project`
  always overrides. The Claude Code plugin drives all of this on its own via a new `SessionStart`
  hook (`vorlyn-ensure.sh`) — installing the plugin is the opt-in, `VORLYN_AUTO=0` the opt-out.
- `upload_document`/`request_review` responses now carry a `url` straight to the document, so an
  ...
  `stdio-main.ts`), or discovered automatically for `vorlyn open`/`vorlyn serve`'s embedded MCP
  ...
  not an opt-in; `VORLYN_AUTO_OPEN_REVIEW=0` turns it off. Deliberately scoped to `request_review`
...
- The Claude Code plugin (`claude-plugin/`, now `0.3.0`) never actually registered an MCP server:
  `hooks/vorlyn-ensure.sh` started the local server and linked the folder, but nothing ever told
  Claude Code's own MCP client an endpoint existed, so `vorlyn-review`'s tools were unreachable
  ...
  only — a remote/team instance's MCP URL isn't derivable from anything `vorlyn link` writes to
