#!/usr/bin/env bash
#
# Vorlyn sync — Claude Code `Stop` hook.
#
# Runs `vorlyn push --quiet` once at the end of an agent turn, so a repo that
# opted into Vorlyn sync never depends on the agent remembering to push. This is
# the *guarantee* layer; the `vorlyn-sync` skill is the *quality* layer (it gets
...
# who never opted in sees no output, ever. `.vorlyn/` is committed, so every
# teammate on a linked repo runs this hook whether or not they use Vorlyn.
#
...
# (b) Never linked (`vorlyn link` was never run here) → silent no-op.
...
# `vorlyn link` installs. When "commit" is in force, that git hook owns the push
...
# diagnosis has to win over every other one. `.vorlyn/` is committed, so every
...
# worse than silence. Anyone who wants the diagnosis runs `vorlyn push` by hand,
# where the CLI's own "No API key found..." is the right message in the right
# place.
if [ -z "${VORLYN_API_KEY:-}" ] && [ ! -f "$REPO_ROOT/.vorlyn/credentials" ]; then
  exit 0
fi

# (e) Locate the CLI. `vorlyn` is not published to a registry yet, so
# VORLYN_CLI_PATH is the escape hatch for a monorepo build that is not on PATH.
...
if [ -z "$VORLYN" ]; then
  hint '.vorlyn/ is linked but the vorlyn CLI is not on PATH — see claude-plugin/README.md'
  exit 1
fi
...
"$VORLYN" push --quiet >/dev/null
PUSH_STATUS=$?
if [ "$PUSH_STATUS" -ne 0 ]; then
  # Bounded self-healing: the one failure mode worth an automatic retry is a
  # local daemon that died mid-session (`vorlyn serve stop`, a crash, a
  # reboot). `vorlyn serve status` is the cheap check; `vorlyn serve start`
  ...
  PUSH_STATUS=$?
  if [ "$PUSH_STATUS" -ne 0 ]; then
    hint "vorlyn push exited $PUSH_STATUS — sync did not complete"
    exit 1
  fi
fi
