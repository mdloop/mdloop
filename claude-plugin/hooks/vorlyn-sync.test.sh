#!/usr/bin/env bash
#
# Branch tests for hooks/vorlyn-sync.sh. Plain bash — this bundle is outside
# `packages/`, so it is deliberately not part of `pnpm verify`'s vitest run.
#
#   bash claude-plugin/hooks/vorlyn-sync.test.sh
#
# Each case builds a throwaway git repo and (where relevant) a stub `vorlyn` on
...
unset VORLYN_API_KEY VORLYN_CLI_PATH
...
link_repo() {
  ...
  cat >"$WORK/repo/.vorlyn/manifest.json" <<'JSON'
...
set_trigger() { # <commit|agent-turn>
  printf '{ "trigger": "%s" }\n' "$1" >"$WORK/repo/.vorlyn/config.json"
}

stub_vorlyn() { # <exit code> — records argv + cwd, always writes a stderr line
  cat >"$WORK/bin/vorlyn" <<STUB
#!/usr/bin/env bash
printf 'argv:%s cwd:%s\n' "\$*" "\$(pwd)" >>"$WORK/invocations"
printf 'stub vorlyn stderr\n' >&2
exit $1
STUB
...
# ---------------------------------------------------------------------------
echo '1. no .vorlyn/ (never linked) → silent no-op'
new_repo
...
# ---------------------------------------------------------------------------
echo '2. trigger: "commit" → silent no-op, vorlyn never invoked'
new_repo
link_repo
printf '{ "include": ["docs/**/*.md"], "trigger": "commit" }\n' >"$WORK/repo/.vorlyn/config.json"
stub_vorlyn 0
run_hook PATH="$WORK/bin:$CLEAN_PATH" VORLYN_API_KEY=vorlyn_test
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
assert_eq 'no stderr' '' "$STDERR"
...
run_hook PATH="$WORK/bin:$WORK/nojq" VORLYN_API_KEY=vorlyn_test
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stderr' '' "$STDERR"
assert_no_vorlyn 'fallback parse saw trigger=commit'
cleanup
...
run_hook PATH="$WORK/bin:$CLEAN_PATH" VORLYN_API_KEY=vorlyn_test
assert_eq 'exit 0' 0 "$STATUS"
assert_contains 'vorlyn invoked' 'argv:push --quiet' "$(cat "$WORK/invocations")"
cleanup
...
# ---------------------------------------------------------------------------
echo '3. linked, has a key, but vorlyn not on PATH → stderr line, exit 1 (visible)'
...
link_repo
set_trigger agent-turn
run_hook PATH="$CLEAN_PATH" VORLYN_API_KEY=vorlyn_test
assert_eq 'exit 1, not 0 (0 would discard the message) and not 2 (2 blocks)' 1 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
assert_contains 'hint names the CLI' 'vorlyn CLI is not on PATH' "$STDERR"
...
link_repo
set_trigger agent-turn
run_hook -u VORLYN_API_KEY PATH="$CLEAN_PATH"
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
...
set_trigger agent-turn
stub_vorlyn 0
mv "$WORK/bin/vorlyn" "$WORK/vorlyn-binary"
run_hook PATH="$CLEAN_PATH" VORLYN_API_KEY=vorlyn_test VORLYN_CLI_PATH="$WORK/vorlyn-binary"
assert_eq 'exit 0' 0 "$STATUS"
assert_contains 'invoked via VORLYN_CLI_PATH' 'argv:push --quiet' "$(cat "$WORK/invocations")"
cleanup
...
# ---------------------------------------------------------------------------
echo '4b. vorlyn exits non-zero (conflict/failure) → hook exits 1, never 2'
new_repo
link_repo
set_trigger agent-turn
stub_vorlyn 1
...
assert_eq 'CLI stderr is the first line' 'stub vorlyn stderr' "$(printf '%s\n' "$STDERR" | head -1)"
assert_contains 'summary names the exit code' 'vorlyn push exited 1' "$STDERR"
cleanup

# ---------------------------------------------------------------------------
echo '4c. vorlyn killed (exit 143, nothing on stderr) → hook still surfaces it'
new_repo
link_repo
set_trigger agent-turn
cat >"$WORK/bin/vorlyn" <<'STUB'
...
assert_contains 'summary is the only diagnostic there is' 'vorlyn push exited 143' "$STDERR"
cleanup
...
# ---------------------------------------------------------------------------
echo '5. linked, vorlyn present, no API key anywhere → silent no-op'
new_repo
link_repo
set_trigger agent-turn
stub_vorlyn 0
run_hook -u VORLYN_API_KEY PATH="$WORK/bin:$CLEAN_PATH"
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
assert_eq 'no stderr' '' "$STDERR"
assert_no_vorlyn 'vorlyn not invoked'
...
# ---------------------------------------------------------------------------
echo '5b. .vorlyn/credentials alone is enough'
new_repo
link_repo
set_trigger agent-turn
stub_vorlyn 0
printf '{ "apiKey": "vorlyn_test" }\n' >"$WORK/repo/.vorlyn/credentials"
run_hook -u VORLYN_API_KEY PATH="$WORK/bin:$CLEAN_PATH"
assert_eq 'exit 0' 0 "$STATUS"
assert_contains 'vorlyn invoked' 'argv:push --quiet' "$(cat "$WORK/invocations")"
cleanup
