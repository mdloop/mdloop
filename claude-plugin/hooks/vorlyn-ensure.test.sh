#!/usr/bin/env bash
#
# Branch tests for hooks/vorlyn-ensure.sh. Plain bash, same idiom as
# vorlyn-sync.test.sh (which see for the general shape) — this bundle is
# outside `packages/`, so it is deliberately not part of `pnpm verify`'s
# vitest run.
#
#   bash claude-plugin/hooks/vorlyn-ensure.test.sh
#
# Each case builds a throwaway git repo and (where relevant) a stub `vorlyn`
...
HOOK="$HOOK_DIR/vorlyn-ensure.sh"

# A PATH with the system tools the hook needs but never a real `vorlyn`, so a
...
unset VORLYN_CLI_PATH VORLYN_AUTO VORLYN_API_KEY
...
assert_no_vorlyn() {
  if [ -f "$WORK/invocations" ]; then
    fail "$1 — vorlyn was invoked: $(cat "$WORK/invocations")"
  else
    ok "$1"
  fi
}
...
link_repo() { # <endpoint> — writes .vorlyn/manifest.json with that endpoint
  mkdir -p "$WORK/repo/.vorlyn"
  printf '{ "endpoint": "%s", "projectId": "p_test", "files": {} }\n' "$1" \
    >"$WORK/repo/.vorlyn/manifest.json"
}
...
stub_vorlyn() { # <serve-status-exit-code>
  cat >"$WORK/bin/vorlyn" <<STUB
#!/usr/bin/env bash
printf 'argv:%s\n' "\$*" >>"$WORK/invocations"
...
STUB
  chmod +x "$WORK/bin/vorlyn"
}

# Records every call's argv as one line in a separate log from the vorlyn
...
assert_no_vorlyn 'vorlyn not invoked'
cleanup

# ---------------------------------------------------------------------------
echo '2. VORLYN_AUTO=0 → the opt-out, silent, exit 0'
new_repo
stub_vorlyn 1
run_hook PATH="$WORK/bin:$CLEAN_PATH" VORLYN_AUTO=0
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
assert_eq 'no stderr' '' "$STDERR"
assert_no_vorlyn 'vorlyn not invoked'
...
# ---------------------------------------------------------------------------
echo '3. vorlyn not on PATH nor VORLYN_CLI_PATH → silent, exit 0 (no nag before opt-in)'
new_repo
...
# ---------------------------------------------------------------------------
echo '3b. VORLYN_CLI_PATH is the fallback when vorlyn is not on PATH'
new_repo
stub_vorlyn 0
run_hook PATH="$CLEAN_PATH" VORLYN_CLI_PATH="$WORK/bin/vorlyn"
assert_eq 'exit 0' 0 "$STATUS"
assert_contains 'invoked via VORLYN_CLI_PATH' 'argv:serve status' "$(cat "$WORK/invocations")"
cleanup

# ---------------------------------------------------------------------------
echo '4. already linked to a non-local endpoint → never autostart a local daemon, silent, exit 0'
new_repo
link_repo 'https://vorlyn.example.com/mcp'
stub_vorlyn 1
run_hook PATH="$WORK/bin:$CLEAN_PATH"
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'no stdout' '' "$STDOUT"
...
assert_contains 'hint names the problem' 'could not start the local Vorlyn server' "$STDERR"
INVOCATIONS=$(cat "$WORK/invocations")
case "$INVOCATIONS" in
...
run_hook PATH="$WORK/bin:$CLEAN_PATH"
assert_eq 'exit 1' 1 "$STATUS"
assert_contains 'hint names the problem' 'vorlyn link failed' "$STDERR"
cleanup
...
link_repo 'http://127.0.0.1:54321/mcp'
stub_vorlyn 0
stub_claude 'vorlyn: http://127.0.0.1:54321/mcp (HTTP) - ✔ Connected'
run_hook PATH="$WORK/bin:$CLEAN_PATH"
assert_eq 'exit 0' 0 "$STATUS"
assert_eq 'vorlyn: only the pre-existing "serve status" from step (e) — the' \
...
chmod +x "$WORK/bin/vorlyn"
mkdir -p "$WORK/repo/.vorlyn"
printf '{"apiKey":"vorlyn_test_fixture"}\n' >"$WORK/repo/.vorlyn/credentials"
...
assert_contains 'registered against the live endpoint' \
  'mcp add --transport http vorlyn http://127.0.0.1:54321/mcp' "$CLAUDE_LOG"
assert_contains 'carried the credentials-file token as a bearer header' \
  'Authorization: Bearer vorlyn_test_fixture' "$CLAUDE_LOG"
assert_contains 'used --scope local, never project (never .mcp.json)' '--scope local' "$CLAUDE_LOG"
cleanup

# ---------------------------------------------------------------------------
echo '9c. VORLYN_API_KEY env var wins over a .vorlyn/credentials file present at the same time'
new_repo
link_repo 'http://127.0.0.1:54321/mcp'
cat >"$WORK/bin/vorlyn" <<STUB
#!/usr/bin/env bash
...
STUB
chmod +x "$WORK/bin/vorlyn"
printf '{"apiKey":"vorlyn_from_file"}\n' >"$WORK/repo/.vorlyn/credentials"
stub_claude ''
run_hook PATH="$WORK/bin:$CLEAN_PATH" VORLYN_API_KEY=vorlyn_from_env
CLAUDE_LOG=$(cat "$WORK/claude-invocations")
assert_contains 'env var value used' 'Bearer vorlyn_from_env' "$CLAUDE_LOG"
case "$CLAUDE_LOG" in
  *"vorlyn_from_file"*) fail 'the credentials-file key leaked in despite VORLYN_API_KEY being set' ;;
...
# ---------------------------------------------------------------------------
echo '9f. VORLYN_AUTO=0 → MCP registration never attempted either'
new_repo
stub_vorlyn 1
stub_claude ''
run_hook PATH="$WORK/bin:$CLEAN_PATH" VORLYN_AUTO=0
...
if grep -q 'vorlyn-ensure\.sh' "$HOOK_DIR/hooks.json"; then
  ok 'points at vorlyn-ensure.sh'
else
  fail 'hooks.json should point the SessionStart hook at vorlyn-ensure.sh'
fi
