#!/usr/bin/env bash
#
# Vorlyn ensure — Claude Code `SessionStart` hook.
#
# Makes "install the plugin, never type `vorlyn open`" true. Where
# `vorlyn-sync.sh` pushes an already-linked repo, this script's job is
# upstream of that: make sure a local Vorlyn server is running, this folder
...
# script**, deliberately — `vorlyn-sync.sh`'s "no manifest → silent exit 0"
...
# (`.vorlyn/manifest.json`'s endpoint is the API base used for pushes, and
...
#
# Exit codes, same contract as `vorlyn-sync.sh` documents in full: 0 is a
...
#
# `set -e` is deliberately absent, matching `vorlyn-sync.sh`: every failure
# path here is an explicit exit, never an arbitrary abort partway through.
set -u

hint() {
  printf 'vorlyn-ensure: %s\n' "$1" >&2
}
...
# VORLYN_AUTO=0 is the escape hatch back out of it.
[ "${VORLYN_AUTO:-1}" = "0" ] && exit 0

# (c) Locate the CLI. Unlike `vorlyn-sync.sh`'s exit-1-with-hint for the same
# condition, a missing CLI here is silent exit 0: `vorlyn-sync.sh` only ever
...
# will. Nagging before anyone has opted in by installing `vorlyn` itself
...
# jq-else-sed idiom `vorlyn-sync.sh` uses to read `.trigger` from
...
MANIFEST="$REPO_ROOT/.vorlyn/manifest.json"
if [ -f "$MANIFEST" ]; then
...
# (e) Ensure the local daemon is up. `vorlyn serve status` is the
...
"$VORLYN" serve status >/dev/null 2>&1 || "$VORLYN" serve start >/dev/null 2>&1 || {
  hint "could not start the local Vorlyn server — see the data directory's serve.log"
  exit 1
}

# (f) Ensure this folder is linked. Only needed the first time a folder is
# opened — once `.vorlyn/manifest.json` exists, later sessions skip straight
# past this. `vorlyn link` with no `--project` auto-provisions against a
...
if [ ! -f "$MANIFEST" ]; then
  cd "$REPO_ROOT" && "$VORLYN" link >/dev/null || {
    hint 'vorlyn link failed'
    exit 1
  }
fi
...
# server exists at all, so the `vorlyn-review` skill's tools were simply
...
# existence check is what makes that true — bare `vorlyn:` is a manually
...
"$CLAUDE_BIN" mcp list 2>/dev/null | grep -q '^vorlyn:' && exit 0

STATUS_JSON=$("$VORLYN" serve status --json 2>/dev/null) || exit 0
if command -v jq >/dev/null 2>&1; then
...
# Same VORLYN_API_KEY-then-.vorlyn/credentials precedence as
# resolveApiKey() in packages/cli/src/credentials.ts.
API_KEY="${VORLYN_API_KEY:-}"
if [ -z "$API_KEY" ] && [ -f "$REPO_ROOT/.vorlyn/credentials" ]; then
  if command -v jq >/dev/null 2>&1; then
    API_KEY=$(jq -r '.apiKey // empty' "$REPO_ROOT/.vorlyn/credentials" 2>/dev/null) || API_KEY=""
  else
    API_KEY=$(sed -n 's/.*"apiKey"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$REPO_ROOT/.vorlyn/credentials" | head -1)
  fi
fi
[ -n "$API_KEY" ] || exit 0
...
# into git).
"$CLAUDE_BIN" mcp add --transport http vorlyn "$MCP_ENDPOINT" \
  --header "Authorization: Bearer $API_KEY" --scope local >/dev/null 2>&1 || {
  hint "could not register the vorlyn MCP server with Claude Code"
  exit 1
}
