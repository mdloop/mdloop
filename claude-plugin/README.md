...
| `.mcp.json`                    | **Primary MCP registration.** Declares the `mdloop` server (`${MDLOOP_MCP_URL:-http://127.0.0.1:58744/mcp}`, bearer token from `${MDLOOP_API_KEY:-}`) so `mdloop-review`'s tools are reachable the moment the plugin loads — no hook, no `claude mcp add`, whenever `MDLOOP_API_KEY` is exported in the environment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `hooks/mdloop-ensure.sh`       | **Zero-touch provisioning, plus the one gap `.mcp.json` can't close.** A `SessionStart` hook: starts the local mdloop server if it isn't already running, links the current repo (auto-provisioning a project named after the folder) if it isn't already linked, and — only when the API key lives in a per-repo `.mdloop/credentials` file rather than the environment, which `.mcp.json`'s `${MDLOOP_API_KEY:-}` has no way to read — registers the running instance with Claude Code's own MCP client (`claude mcp add`, `--scope local`) as a fallback so `mdloop-review`'s tools are reachable anyway; a fast existence check makes every later session free. This is what makes "install the plugin, never type `mdloop open`" true — see "Usage" below. Opt out with `MDLOOP_AUTO=0`. Never touches a repo already linked to a non-local server; the fallback registration is local-only for the same reason (a remote endpoint's MCP URL isn't derivable from anything on disk). |
...
shape: the skill itself is MCP-only and needs no shell at all, and — now that `.mcp.json` declares the
server up front — usually neither do the _tools it depends on_ to become reachable. The one case that
still needs a shell is a per-repo `.mdloop/credentials` key `.mcp.json` cannot read; that's exactly what
`hooks/mdloop-ensure.sh`'s fallback registration step is for. (An earlier version of this paragraph
reasoned "so needs no hook" from the first half alone, and missed that: a hook can't _call_ an MCP tool,
but it can still _register_ one — a plain CLI/config operation, no different in kind from starting the
server or linking the folder above it.)
...
The `mdloop` CLI must be reachable on `PATH`. Easiest path:
...
1. Open Claude Code in a repo. `.mcp.json` (bundled with the plugin) declares the `mdloop` MCP
   server up front, so `mdloop-review`'s tools are reachable from the very first turn whenever
   `MDLOOP_API_KEY` is exported in the environment — no hook involved. The `SessionStart` hook
   (`hooks/mdloop-ensure.sh`) then starts the local mdloop server if it isn't already running
   (`mdloop serve start`, detached — it survives closing the terminal) and links the repo if it
   isn't already linked (`mdloop link` with no `--project`, which auto-provisions a project named
   after the folder, reusing one if it already exists).
2. That's it. From here on this is the same loop as the shared-instance path below — commits push, the
   ...
   applies only when the API key lives in a per-repo `.mdloop/credentials` file rather than the
   environment — `.mcp.json`'s `${MDLOOP_API_KEY:-}` has no way to read that, so the hook falls
   back to registering with Claude Code's own MCP client (`claude mcp add ... --scope local`)
   instead. That fallback registration happens on the session that runs the very first `mdloop
link` for a repo, and Claude Code's own docs don't guarantee a hook-registered server is
   reachable in that same session — so the read half may need one fresh session to activate the
   first time such a repo is opened. Every session after that already has a working registration
   sitting in `~/.claude.json`, checked with one cheap `claude mcp list` call and nothing else — the
   admin key doesn't rotate, so there's nothing left to redo. `claude mcp list` / `claude mcp get
mdloop` shows the live registration at any point. Local only, deliberately: a shared instance's
   MCP URL isn't derivable from anything `mdloop link` writes to disk (see "A real shared instance"
   below), so that path still ends with one manual `claude mcp add` — same as any other MCP client.
