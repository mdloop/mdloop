---
name: vorlyn-sync
description: Use when a significant change has been made to a markdown spec, PRD, design doc, plan, or ADR in a repo with a .vorlyn/ directory — at meaningful mid-session checkpoints as well as when finishing a round of edits. Also use when asked to "push to Vorlyn", "sync the spec", "send this for review", or similar.
---

# Pushing an artifact to Vorlyn

## Overview

Vorlyn is where humans read, comment on, and sign off the artifacts an agent writes. `vorlyn push` creates
...
regardless of which coding agent or editor made the edit. The consequence: between commits, Vorlyn goes
...
a nicety under this default — it is the only thing keeping Vorlyn current mid-session.

(If `.vorlyn/config.json` sets `"trigger": "agent-turn"`, a Stop hook also pushes at the end of every turn
...
```bash
vorlyn push --note "<what changed and why>"
```

from the repo root (the folder containing `.vorlyn/`).
...
| Change is to code, not a tracked markdown artifact                | No — Vorlyn tracks `*.md` only                                                  |
| No `.vorlyn/` directory in the repo                               | Yes — see "Make sure there is somewhere to push to" below before the first push |
...
`vorlyn push` is safe to run at any time: unchanged files are skipped without a network call, and a push
that would clobber someone else's newer version refuses with a conflict instead (reconcile, then consider
`--force`). `vorlyn status` shows what would push without pushing anything.

## Make sure there is somewhere to push to

A `SessionStart` hook normally already did this — it starts the local Vorlyn server if needed and links
the repo, so by the time you're pushing there is almost always already a `.vorlyn/manifest.json`. Do it
yourself only when that hook didn't run or didn't finish: `VORLYN_AUTO=0` disables it, or you moved into a
different repo mid-session (the hook only ran once, at session start, in whatever folder that was).

```bash
vorlyn serve status || vorlyn serve start
vorlyn link
```

`vorlyn serve status` exits 0 if a local server is already running, so `vorlyn serve start` only runs
when it's genuinely needed. Then `vorlyn link`:

- **Reuse before create.** With no `--project` flag, `vorlyn link` first looks for an existing project
  whose name matches this folder and reuses it — it does not create a new project unless nothing matches.
  `vorlyn projects list` shows every folder→project mapping this machine has ever made, so you can check
...
- **A human-provisioned project always wins.** If the user has already created or named a specific project,
  link to it explicitly with `vorlyn link --project <id>` rather than letting auto-provisioning pick one.
  Never invent a project id, and never route around linking by calling `upload_document` directly just
  because `.vorlyn/` doesn't exist yet — that desynchronizes the CLI's file→document mapping (see "Common
  mistakes" below).
- **Local only.** Auto-creating a project like this is restricted to a **local** Vorlyn server. Against a
  shared team instance, don't let `vorlyn link` auto-provision — ask the human which existing project this
...
- **Diagrams and tables are first-class.** Vorlyn renders mermaid, code fences, and callouts — structure
...
| Saving every push for the very end of a long session                           | Vorlyn stays stale the whole session under the default trigger — push at checkpoints along the way                                                                                      |
| Told not to stop and sync mid-session ("we're on a deadline, just keep going") | That's a request not to interrupt the work, not a request to skip Vorlyn for the rest of the session — keep working uninterrupted, then push what accumulated the next time you do stop |
