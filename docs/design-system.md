...
Direction: track-and-relay vernacular — Vorlyn is the handoff between people
...
| `--vorlyn`       | `#2456e6`     | The single action accent: buttons, focus, active    |
...
echoing the relay metaphor end to end — retired because it depended on the
"Vorlyn" relay-race identity, which is itself under review (see
`docs/RISKS.md` — the product name is unresolved); "Leg n" reads as
unexplained slang once that metaphor no longer holds. An intermediate
...
- Focus: 2px `--vorlyn` outline, visible on every interactive element.
