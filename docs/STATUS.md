...
- **20 MCP tools**, generated into `docs/reference/mcp-reference.md` by `pnpm gen:mcp`; `pnpm docs:check`
  ...
  newest: `vorlyn link`/`vorlyn open` use it to give each folder its own auto-provisioned
  project against a local instance instead of sharing one machine-wide default.
- **`vorlyn serve start|stop|status`** — a detached, persistent local instance (survives closing
  the terminal that started it), so multiple folders/projects on one machine share a single
  embedded Postgres + server instead of each `vorlyn open` fighting over the same ports.
  `vorlyn open` attaches to one that's already running instead of starting a second. Both the API
  ...
  `SessionStart` hook (`vorlyn-ensure.sh`) — installing the plugin is the opt-in, `VORLYN_AUTO=0`
  ...
  `vorlyn-review`'s skill assuming they were.
...
- **`vorlyn serve`'s liveness check has no positive identity signal.** It confirms a recorded pid
