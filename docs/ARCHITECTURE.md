...
`VORLYN_AUTH_MODE=single-user` (one fixed admin identity, no login screen) and `=oidc` (real
...
packages/
  ...
  cli/           # `vorlyn` CLI — link/unlink/push/status/watch (repo <-> document sync),
...
1. **Postgres RLS** — every tenant-scoped table carries `org_id`; policy `USING (org_id = current_setting('app.org_id')::uuid)`. App connects as `vorlyn_app` (non-superuser, `NOBYPASSRLS`). Policies are `FORCE ROW LEVEL SECURITY`.
...
Tested per phase: org A vs org B via HTTP API, MCP tool, and direct repository call, against an ephemeral per-run Postgres database (`createTestDb` in `@vorlyn/persistence/test-support`).
...
The agent-side companion is `claude-plugin/skills/vorlyn-review/SKILL.md`, which teaches the read half of
this loop and states the same premise from the other direction ("This skill assumes **nothing local
exists**: no repo, no checkout, no `.vorlyn/`"). It also carries the one branch that matters here: if the
agent _does_ happen to sit in a repo containing `.vorlyn/`, the push step belongs to `vorlyn push` so the
...
- vorlyn.md registrar + trademark check before public anything.
