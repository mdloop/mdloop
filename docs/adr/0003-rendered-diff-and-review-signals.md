...
Both reviews independently converge on cutting the GitHub-style pending-comment batch. Architecture: a draft flag must be filtered in _every_ comment read path (list, counts, status rollup, feedback bundle, minimap, search, the hard approval gate — whose open-count a reviewer's own drafts would otherwise deadlock); one missed predicate leaks a reviewer's unsent notes cross-user. UX: a once-a-week reviewer expects Post → sent, and **Vorlyn already has its batch-submit moment — the sign-off verdict with note** (ADR 0002). Building a second competing "pending" concept adds developer-tool vocabulary for negative value. Revisit only on demonstrated power-user demand, opt-in, and only while a review is in progress.
