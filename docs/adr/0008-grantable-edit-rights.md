...
- Phase 20's sync CLI becomes usable by any grantee, not just the document owner — which is the entire point. A teammate cloning the repo needs only their own `MDLOOP_API_KEY` plus an `edit` grant on the documents they push.
