...
- New DB role `mdloop_public_reader` (`nologin nobypassrls`) is granted `SELECT` on `public_documents`
...
- Publishing is the **single bridge** between the tenant model and the public store: a home-org admin
  ...
  write a copy into the public table/keyspace via the existing `mdloop_provisioner` role. The public read
