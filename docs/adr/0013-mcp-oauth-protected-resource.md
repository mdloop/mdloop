...
in July, pre-2026-08-09) assumed mdloop would have to build its own OAuth 2.1 authorization server
...
- `mdloop_`-prefixed → the existing `actorForApiKey` path, completely unchanged. This stays
...
- anything else → a new `actorForOAuthToken` path: verify the token as a WorkOS AuthKit-issued
  ...
  restricted to an explicit allowlist), then resolve its `sub` claim to a mdloop user via the
...
- **Build the full authorization server as originally spec'd.** Rejected: strictly more code,
  ...
  mdloop's own requirements needs mdloop to be the AS.
