...
import { API_KEY_PREFIX, UnlimitedRateLimiter, actorForApiKey } from '@vorlyn/app';
 ...
 * `vorlyn open ./folder` (packages/cli/src/open.ts) spawns, sibling to
 * `@vorlyn/api`'s `selfhost-embedded-main.ts`. Deliberately near-identical
 * to `main.ts` (same telemetry/pool/fault-handling/`/healthz`+`/readyz`/
 * `/mcp` shape) with the same two categories of difference `@vorlyn/api`'s
 ...
 *      role vorlyn_app`). `main.ts`'s own call is untouched.
 ...
 *      coordinate across. Auth is the plain `vorlyn_`-prefixed API-key path
 ...
 * listens on its own port, e.g. 3001, independent of `@vorlyn/api`'s 3000 —
 * `packages/cli/src/link.ts`'s `DEFAULT_ENDPOINT` already assumes this
 * split). `vorlyn open`'s CLI-driven push/link step talks to Vorlyn over MCP
 ...
 * as it needs an embedded-mode HTTP API — without this file, `vorlyn link`/
 * `vorlyn push` would have nothing local to connect to.
 */
const env = mcpHttpEnvFromEnv();

await setupTelemetryIfAvailable('vorlyn-mcp-selfhost-embedded');
...
// boot, so an MCP endpoint URL stays valid across a `vorlyn serve`
...
// below for `vorlyn`'s CLI-side supervisor to read either way.
...
/**
 * Same opt-out shape as `VORLYN_AUTO` (`claude-plugin/hooks/vorlyn-ensure.sh`):
 ...
 * not an opt-in extra.
 */
const autoOpenReview = process.env.VORLYN_AUTO_OPEN_REVIEW !== '0';

/**
 * `upload_document`/`request_review`'s `url` field (see `McpDeps.webAppUrl`)
 * needs the sibling `@vorlyn/api` embedded child's port — a process this
...
/** Only the `vorlyn_`-prefixed API-key path — no OAuth branch (difference 2 above). */
...
// not always OS-assigned — see @vorlyn/cli's `DEFAULT_MCP_PORT`) is the
...
http.listen(port, '127.0.0.1', () => {
  ...
  // eslint-disable-next-line no-console
  console.log(`vorlyn mcp (embedded) on :${String(boundPort)}/mcp`);
  // Announced for the CLI-side supervisor (packages/cli/src/local-instance.ts)
  // to discover — see the matching comment on @vorlyn/api's embedded main.
  // Same fail-fast posture as that file: VORLYN_DATA_DIR is set by the CLI
  // on every spawn of this process, never a human-edited .env.
  const dataDir = process.env.VORLYN_DATA_DIR;
  if (!dataDir) {
    throw new Error(
      'VORLYN_DATA_DIR must be set — this entrypoint is only ever spawned by the CLI',
    );
  }
