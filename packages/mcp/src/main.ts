...
import { API_KEY_PREFIX, UserRateLimiter, actorForApiKey, actorForOAuthToken } from '@mdloop/app';
...
/**
 * MCP over streamable HTTP. Auth: `Authorization: Bearer mdloop_...` per-user
...
// AuthKit-issued token to a mdloop user — not part of McpDeps, since no MCP
...
/** Bearer-token dispatch: `mdloop_`-prefixed → the existing API-key path
...
http.listen(port, '0.0.0.0', () => {
  ...
  console.log(`mdloop mcp on :${String(port)}/mcp`);
});
