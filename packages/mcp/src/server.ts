...
import type { CommentId, DocumentId, ProjectId, ReplyId, UserId } from '@vorlyn/shared';
...
export interface McpDeps {
   ...
   * `http://127.0.0.1:60356` or `https://vorlyn.example.com` — no trailing
   ...
   * the reviewer's own browser straight to it (`vorlyn open`/`vorlyn
...
export function buildMcpServer(deps: McpDeps, actor: Actor): McpServer {
  const server = new McpServer({ name: 'vorlyn', version: '0.1.0' });
    ...
    'upload_document',
    {
      ...
      inputSchema: {
        ...
        path: z
          ...
          .optional()
          .describe(
            'Repo-relative POSIX path including the filename, e.g. "docs/specs/auth.md", when mirroring a real file so Vorlyn can show the folder structure. Sending an existing document_id with a DIFFERENT path moves the document there rather than forking a duplicate; omitting it leaves any stored path untouched (there is no way to clear one). Max 1024 characters, no absolute paths or ".." traversal. Omit unless you are mirroring a repo.',
          ),
      },
    },
    tool(
    ...
    'request_review',
    {
      ...
      inputSchema: {
        ...
        reviewer_user_id: z
          .string()
          .describe(
            "Uuid of the Vorlyn user being asked — a users.id, not an email or display name. Reviewer ids appear in get_review_status's `requests` and `approvals`. They must already have access to the document.",
          ),
      },
    },
