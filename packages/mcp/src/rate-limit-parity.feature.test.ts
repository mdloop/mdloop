...
import type { Actor, RateLimitError, RateLimiterPort } from '@mdloop/app';
...
import { TIER_PROFILES, consumeRequest } from '@mdloop/domain';
...
import { createTestDb, createTestRedis } from '@mdloop/persistence/test-support';
import type { TestDb, TestRedis } from '@mdloop/persistence/test-support';
...
describeFeature(feature, ({ Scenario, AfterAllScenarios }) => {
  ...
  async function buildServerAndClient(
    ...
    const dir = await mkdtemp(path.join(tmpdir(), 'mdloop-parity-'));
  ...
  Scenario(
    ...
    ({ Given, When, Then, And }) => {
      Given('organization "Acme" on the free tier with an agent acting as "alice"', async () => {
        ...
        storageDir = await mkdtemp(path.join(tmpdir(), 'mdloop-parity-'));
        const deps: McpDeps = {
