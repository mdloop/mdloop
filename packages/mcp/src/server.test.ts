...
import type { Actor, OAuthTokenVerifierPort } from '@vorlyn/app';
import type { DocumentId, UserId } from '@vorlyn/shared';
import { actorForApiKey, actorForOAuthToken, createApiKey } from '@vorlyn/app';
...
describe('MCP server', () => {
  ...
  beforeAll(async () => {
    ...
    storageDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-mcp-blobs-'));
  ...
  it('resolves an actor from a live API key, end to end', async () => {
    ...
    expect(await actorForApiKey(deps.apiKeys, 'vorlyn_not-a-real-key')).toBeUndefined();
  });
  ...
  it('upload_document and request_review carry a url only when webAppUrl is configured', async () => {
    ...
    const withUrl = await connectAs(ownerActor, { webAppUrl: 'https://vorlyn.example.test' });
    const uploaded = parse(
    ...
    expect(uploaded.url).toBe(`https://vorlyn.example.test/d/${uploaded.document_id}`);
    const requested = parse(
      (await withUrl.callTool({
        name: 'request_review',
        arguments: { document_id: uploaded.document_id, reviewer_user_id: reviewer.id },
    ...
    expect(requested.url).toBe(`https://vorlyn.example.test/d/${uploaded.document_id}`);
    await withUrl.close();
  });
  ...
  it('request_review fires onReviewRequested with the document url, and never on upload_document', async () => {
    ...
    const client = await connectAs(ownerActor, {
      webAppUrl: 'https://vorlyn.example.test',
      onReviewRequested,
    });
    const uploaded = parse(
      (await client.callTool({
    ...
    expect(onReviewRequested).toHaveBeenCalledExactlyOnceWith(
      `https://vorlyn.example.test/d/${uploaded.document_id}`,
    );
    await client.close();
  });
