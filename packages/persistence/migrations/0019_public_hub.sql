...
-- mdloop_public_reader: the ONLY role the unauthenticated public-hub read
...
do $$
begin
  if not exists (select from pg_roles where rolname = 'mdloop_public_reader') then
    create role mdloop_public_reader nologin nobypassrls;
  end if;
end
$$;
grant usage on schema public to mdloop_public_reader;
grant select on public_documents to mdloop_public_reader;
...
-- snapshot via the existing mdloop_provisioner role.
grant select, insert, update, delete on public_documents to mdloop_provisioner;
