-- Phase 26.A: mdloop operator admin console — auth/audit foundation.
--
-- `operators` is a mdloop-staff identity, deliberately NOT a tenant table: no
-- org_id, no RLS (there is no tenant to scope to — an operator's whole point
-- is cross-org reach). It is reached exclusively through mdloop_provisioner,
...
-- reads in 0008). mdloop_app has no grant here at all — ordinary tenant-scoped
...
grant select, insert, update, delete on operators to mdloop_provisioner;
grant select, insert on operator_audit_log to mdloop_provisioner;
