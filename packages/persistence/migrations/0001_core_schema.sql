...
-- The application connects as (or SET ROLEs to) vorlyn_app: non-superuser,
...
do $$
begin
  if not exists (select from pg_roles where rolname = 'vorlyn_app') then
    create role vorlyn_app nologin nobypassrls noinherit;
  end if;
end
$$;
...
grant usage on schema public to vorlyn_app;
grant select, insert, update, delete on all tables in schema public to vorlyn_app;
...
do $$
begin
  if not exists (select from pg_roles where rolname = 'vorlyn_provisioner') then
    create role vorlyn_provisioner nologin bypassrls noinherit;
  end if;
end
$$;
grant usage on schema public to vorlyn_provisioner;
grant select, insert on organizations, users to vorlyn_provisioner;
grant update (role, display_name) on users to vorlyn_provisioner;
