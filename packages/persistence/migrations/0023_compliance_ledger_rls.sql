...
-- were, until now, kept isolated only by grant-absence — vorlyn_app has no
...
-- on the call stack), and vorlyn_provisioner is BYPASSRLS — it never consults
...
-- The policies bind only the non-superuser vorlyn_app role's future access.
