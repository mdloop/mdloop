...
import { publicObjectKey, versionObjectKey } from '@vorlyn/app';
 ...
 * `vorlyn` npm package, which bundles this file (esbuild inlines every
 * `@vorlyn/*` module into one output) while declaring `@aws-sdk/client-s3`
  ...
  awsSdkPromise ??= (async () => {
    try {
      ...
      if (isModuleNotFoundError(error)) {
        throw new Error(
          'VORLYN_BLOBS_BUCKET is set but @aws-sdk/client-s3 is not installed — run ' +
            '"npm i @aws-sdk/client-s3", or unset VORLYN_BLOBS_BUCKET to use local-disk storage.',
        );
      }
      throw error;
    }
...
export class S3Storage implements StoragePort {
   ...
   * in the published `vorlyn` package). The SDK is loaded lazily, once, the
