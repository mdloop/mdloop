import type { StoragePort } from '@vorlyn/app';
 ...
 *
 * `VORLYN_BLOBS_BUCKET` set → `S3Storage` against it (region from
 * `AWS_REGION`, however a deployment injects it).
 * `VORLYN_BLOBS_ENDPOINT`/`VORLYN_BLOBS_FORCE_PATH_STYLE` (both optional,
 ...
 * `NODE_ENV=production` unless `VORLYN_ALLOW_LOCAL_BLOB_STORAGE=true` is also
...
export function storageFromEnv(env: NodeJS.ProcessEnv = process.env): StoragePort {
  const bucket = env.VORLYN_BLOBS_BUCKET?.trim();
  if (!bucket) {
    const allowLocal = env.VORLYN_ALLOW_LOCAL_BLOB_STORAGE?.trim() === 'true';
    if (env.NODE_ENV === 'production' && !allowLocal) {
      throw new Error(
        'VORLYN_BLOBS_BUCKET must be set under NODE_ENV=production, or set ' +
          'VORLYN_ALLOW_LOCAL_BLOB_STORAGE=true to use local-disk storage deliberately ' +
    ...
    return new FsStorage(env.BLOB_STORAGE_DIR ?? '.vorlyn-blobs');
  }
  const endpoint = env.VORLYN_BLOBS_ENDPOINT?.trim();
  return new S3Storage({
    ...
    ...(endpoint ? { endpoint } : {}),
    ...(env.VORLYN_BLOBS_FORCE_PATH_STYLE !== undefined
      ? { forcePathStyle: env.VORLYN_BLOBS_FORCE_PATH_STYLE.trim() === 'true' }
      : {}),
  });
}
