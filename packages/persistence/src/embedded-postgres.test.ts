...
describe('startEmbeddedPostgres', () => {
  ...
  it('binds an OS-assigned ephemeral port and exposes a usable postgres:// connection string', async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-embedded-pg-'));
  ...
  /**
   * The shape `vorlyn open` actually uses, and the one nothing tested until
   * this regressed: it spawns TWO processes (`@vorlyn/api` and `@vorlyn/mcp`
   ...
   * 503 forever and `vorlyn open` timed out on every run, from the day it
   * shipped. Two pools here rather than two processes: same invariant, a
   * fraction of the cost, and it fails for the same reason.
   */
  it('serves two independent clients at once — vorlyn open spawns two processes', async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-embedded-pg-'));
    embedded = await startEmbeddedPostgres(dataDir);
