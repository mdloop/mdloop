 ...
 * session variable bound via SET LOCAL, executing as the mdloop_app role
...
/** Provisioning path: runs as mdloop_provisioner (bypasses RLS for org/user bootstrap only). */
export async function withProvisioner<T>(
  ...
  try {
    await client.query('begin');
    await client.query('set local role mdloop_provisioner');
    await setStatementTimeout(client, statementTimeoutMs);
    const result = await fn(client);
    await client.query('commit');
    return result;
...
/** Public Docs Hub read path (ADR 0004): mdloop_public_reader has SELECT on public_documents only — structurally cannot read tenant tables. The only query path for unauthenticated public reads. */
export async function withPublicReader<T>(
  ...
  try {
    await client.query('begin');
    await client.query('set local role mdloop_public_reader');
    await setStatementTimeout(client, statementTimeoutMs);
