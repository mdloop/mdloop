 ...
 */
describe('vorlyn_login role (migration 0024)', () => {
  let db: TestDb;
  ...
  it('is a LOGIN role that is neither superuser nor BYPASSRLS', async () => {
    const { rows } = await db.pool.query<{
      ...
      rolcanlogin: boolean;
    }>("select rolsuper, rolbypassrls, rolcanlogin from pg_roles where rolname = 'vorlyn_login'");
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ rolsuper: false, rolbypassrls: false, rolcanlogin: true });
  });

  it('is a member of vorlyn_app, vorlyn_provisioner and vorlyn_public_reader', async () => {
    const { rows } = await db.pool.query<{ rolname: string }>(
      `select r.rolname
         from pg_auth_members m
         join pg_roles r on r.oid = m.roleid
         join pg_roles member on member.oid = m.member
        where member.rolname = 'vorlyn_login'
        order by r.rolname`,
    );
    expect(rows.map((r) => r.rolname)).toEqual([
      'vorlyn_app',
      'vorlyn_provisioner',
      'vorlyn_public_reader',
    ]);
  });
});
