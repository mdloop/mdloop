...
import type { AllowlistEntry, OrgInvite } from '@mdloop/domain';
import type { AllowlistEntryId, InviteId, OrgId } from '@mdloop/shared';
...
export class PgOrgInviteRepository implements OrgInviteRepository {
  ...
  async createWithinSeatCapForOrg(
    ...
    return withProvisioner(this.pool, async (c) => {
      // No RLS to lean on here (mdloop_provisioner bypasses it) — every
