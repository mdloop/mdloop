...
export interface EmbeddedPostgres {
   ...
   * not this codebase's `mdloop_login` role (migration 0024). See the
 ...
 * engine for the self-host / `mdloop open` composition root (not wired here —
 ...
 * `mdloop_login` — a dedicated NOSUPERUSER/NOBYPASSRLS login role (migration
 ...
 * `mdloop_login` role machinery reachable "over the wire" the way a real
 * Postgres server's `pg_hba.conf` would gate it. **This does not mean RLS
 * itself is bypassed.** Migrations 0001+0024 create `mdloop_app` /
 * `mdloop_provisioner` / `mdloop_public_reader` as real roles regardless of
 * which engine runs the SQL, and Postgres's `SET ROLE` semantics mean a
 * superuser session that does `set local role mdloop_app` genuinely drops to
 ...
 * first place*: in prod, only `mdloop_login` (a non-superuser) can connect at
 ...
 * `read ECONNRESET`.
 *
 * That default is why this must be set explicitly: `mdloop open` runs TWO
 * processes (`@mdloop/api` and `@mdloop/mcp`'s `selfhost-embedded-main`s)
 * against one instance, so the second one to connect was refused outright —
 * its `/readyz` returned 503 forever and `mdloop open` timed out on every
