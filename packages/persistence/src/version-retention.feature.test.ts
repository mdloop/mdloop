...
import type { Actor, ThreadWithResolution, VersionPurgeDeps } from '@mdloop/app';
import {
  createComment,
...
import type { CommentId, DocumentId } from '@mdloop/shared';
  ...
  Background(({ Given }) => {
    Given(
      ...
      async () => {
        ...
        w.storageDir = await mkdtemp(path.join(tmpdir(), 'mdloop-vp-'));
