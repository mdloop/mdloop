...
import type { DocumentId, OrgId, UserId, VersionId } from '@mdloop/shared';
 ...
 *    removes the row;
 *  - the load-bearing safety proof for the whole ADR — `mdloop_public_reader`
...
describe('public hub (ADR 0004)', () => {
  ...
  it('mdloop_public_reader is structurally incapable of reading documents/document_versions (ADR 0004 safety proof)', async () => {
    // Several possible rejection reasons, all proving the same thing:
    //   1. mdloop_public_reader has no SELECT grant on either table at all
    ...
    // practice, enforced by grant (SELECT only for mdloop_public_reader).
    await expect(
