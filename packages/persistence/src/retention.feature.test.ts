...
import type { Actor, ErasureLogPort } from '@mdloop/app';
import { deleteDocument, sweepDuePurges, uploadNewDocument } from '@mdloop/app';
...
describeFeature(feature, ({ Background, Scenario, AfterAllScenarios, AfterEachScenario }) => {
  ...
  Background(({ Given }) => {
    Given('organization "Acme" with user "alice" and retention of 10 days', async () => {
      ...
      w.storageDir = await mkdtemp(path.join(tmpdir(), 'mdloop-retention-'));
