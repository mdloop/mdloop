...
} from '@mdloop/persistence/embedded';
 ...
 * on the next `mdloop serve stop` + `start`, because the port underneath
 ...
 * explicit way out; `selfhost-embedded-main.ts` in both `@mdloop/api` and
 * `@mdloop/mcp` fails loud and names that escape hatch rather than
...
/** What `startEmbeddedPostgres` (`@mdloop/persistence`) returns — named
...
/**
 * Where `@mdloop/api`'s or `@mdloop/mcp`'s `selfhost-embedded-main` can be
 * found, relative to *this module's own compiled location* — deliberately
 * not resolved via `import.meta.resolve('@mdloop/api')`/`'@mdloop/mcp')`.
 ...
 * moment this CLI is installed from npm: `@mdloop/mcp` was deliberately
 * never a `cli` dependency (`cli-http-only`, see below), so npm never
 * fetches it at all, and even `@mdloop/api`'s resolved path under pnpm's
 ...
 *     (esbuild inlines `local-instance.ts` into it), and `@mdloop/api`'s/
 *     `@mdloop/mcp`'s bundled entrypoints sit at `dist/api/…`, `dist/mcp/…`
 ...
 *
 * `@mdloop/mcp` is still never a `cli` dependency — nothing here needs it
 ...
 * of `@mdloop/mcp`'s code — `cli` talks to MCP over HTTP, as before.
 *
...
export async function resolveEmbeddedEntrypointFrom(
  ...
  const entry = await firstExistingRelative(
    ...
    `@mdloop/${pkg}'s embedded entrypoint`,
    exists,
  );
 ...
 * `/readyz`. The shared core of `mdloop open`'s "own mode" and `mdloop
...
export async function startLocalInstance(
  ...
  try {
    ...
    const sharedEnv: NodeJS.ProcessEnv = {
      ...
      MDLOOP_AUTH_MODE: 'single-user',
      MDLOOP_ADMIN_EMAIL: localConfig.adminEmail,
      MDLOOP_ADMIN_NAME: localConfig.adminName,
    ...
    apiChild = await deps.spawnApi({ ...sharedEnv, MDLOOP_SERVE_WEB: 'true' });
    ...
    // the browser (`mdloop open`) navigates to this exact host, and the
