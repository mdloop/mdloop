...
const IGNORED_DIRS = new Set(['node_modules', '.git', '.vorlyn']);
...
/**
 * `vorlyn watch` — starts a chokidar watcher over `options.folder`'s markdown
 ...
 *
 * Refuses to start against a folder with no `.vorlyn/manifest.json` — the
 * folder must already be linked via `vorlyn link`.
 *
