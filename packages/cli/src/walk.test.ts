...
describe('walkMarkdownFiles', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-walk-'));
    ...
    await mkdir(path.join(folder, '.vorlyn'), { recursive: true });
    ...
    await writeFile(path.join(folder, '.vorlyn', 'manifest.json'), '{}');
  });
  ...
  it('returns an empty list for an empty folder', async () => {
    const empty = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-walk-empty-'));
    try {
  ...
  it('does not descend into ordinary nested subdirectories with no .vorlyn anywhere', async () => {
    const files = await walkMarkdownFiles(folder);
    // Regression coverage: docs/nested/deep.md (an ordinary nested
    // subdirectory two levels down, with no .vorlyn/ anywhere in it) still
    // walks exactly as before.
    expect(files).toContain(path.join('docs', 'nested', 'deep.md'));
  });

  it('walks straight through a nested .vorlyn-linked subdirectory rather than walling it off', async () => {
    ...
    const root = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-walk-nested-'));
    try {
      await mkdir(path.join(root, 'nested', '.vorlyn'), { recursive: true });
      await writeFile(path.join(root, 'a.md'), '# a');
      await writeFile(path.join(root, 'nested', '.vorlyn', 'manifest.json'), '{}');
