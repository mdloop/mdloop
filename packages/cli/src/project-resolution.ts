...
/** The minimal slice of `mcp-client.ts`'s `McpProject`/`VorlynMcpClient` this file needs — named
...
/**
 * "Smart about reuse-vs-create" — the logic `vorlyn link`'s auto-provision
 * path (and so `vorlyn open`, and `vorlyn-ensure.sh`) defers to whenever a
