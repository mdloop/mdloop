 ...
 * when the rename off `mdloop` (gated on Gate 0 legal/naming clearance)
 ...
 */
const APP_DIRNAME = 'mdloop';

/**
 * Resolves the standard per-OS application-data directory for this app —
 * where the self-host / `mdloop open` composition root's embedded Postgres
 * (Task 1, `packages/persistence/src/embedded-postgres.ts`) and blob storage
 * live.
 *
 * - Explicit override: `MDLOOP_DATA_DIR`, if set — checked first, before any
 ...
 * - macOS:   `~/Library/Application Support/mdloop/`
 * - Linux:   `$XDG_DATA_HOME/mdloop/` if set, else `~/.local/share/mdloop/`
 * - Windows: `%APPDATA%\mdloop\`
 *
...
export function resolveDataDir(
  ...
  const override = env.MDLOOP_DATA_DIR?.trim();
