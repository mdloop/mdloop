import { readVorlynConfig } from './vorlyn-config.js';
import { gitHookStatus } from './git-hook.js';

/**
 * Closes a specific silent gap: `vorlyn link` run before `git init` skips
 ...
 * `git init` after the fact). `.vorlyn/config.json` still says `trigger:
 ...
 * `claude-plugin/hooks/vorlyn-sync.sh`). The result is a linked folder that
 ...
 *   `link` already had its own moment to report (re-running `vorlyn link`
 *   would not fix it, since there is still no git to hook);
 * - `gitHookStatus` is `'foreign'` or `'vorlyn_managed'` — a hook already
...
export async function commitTriggerMissingHookWarning(folder: string): Promise<string | undefined> {
  const config = await readVorlynConfig(folder);
  // No config file yet is the CLI-side default before anyone edited
  // anything — same tolerant fallback `readVorlynConfig` itself applies.
  ...
  return (
    ...
    'automatically. Re-run "vorlyn link" to install it (this can happen if link ran ' +
    'before "git init").'
  );
}
