...
/**
 * `.mdloop/credentials` — never in the manifest, never committed. The
 * "never committed" half is enforced, not just documented: every writer of
 * `.mdloop/` routes through `ensureMdloopDir`, which guarantees the nested
...
export function credentialsPath(folder: string): string {
  return path.join(mdloopDir(folder), CREDENTIALS_FILENAME);
}
...
/**
 * Writes `.mdloop/credentials` at mode 0600 regardless of the process umask.
...
/**
 * Resolution order: `MDLOOP_API_KEY` env var, else `.mdloop/credentials` in
...
export async function resolveApiKey(
  ...
  const fromEnv = env.MDLOOP_API_KEY;
 ...
 * self-host deploy's admin would use). Both `mdloop open` (unconditionally)
 * and `mdloop link --autoProvision` (only against a local endpoint) fall
