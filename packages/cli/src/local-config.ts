...
/**
 * `<dataDir>/config.json` — the one piece of state `vorlyn open` needs across
 ...
 * folder — unlike `.vorlyn/manifest.json`/`credentials`, this is instance
 * state, not per-folder state, and a data directory backs every folder
 * `vorlyn open` is ever pointed at from this machine.
 */
 ...
 * prior `vorlyn open` run against this data directory), otherwise generates
 ...
 * `credentials.ts`'s `.vorlyn/credentials`).
 *
 * A session secret that changed on every run would invalidate every open
 * browser session each time `vorlyn open` restarted — the entire reason this
