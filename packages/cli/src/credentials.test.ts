...
describe('credentials', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-creds-'));
  });
  ...
  it('round-trips via readCredentials', async () => {
    await writeCredentials(folder, { apiKey: 'vorlyn_y' });
    expect(await readCredentials(folder)).toEqual({ apiKey: 'vorlyn_y' });
  });
  ...
  it('writes the credentials file with mode 0600', async () => {
    await writeCredentials(folder, { apiKey: 'vorlyn_x' });
  ...
  describe('resolveApiKey precedence', () => {
    ...
    it('resolves from the credentials file when present', async () => {
      await writeCredentials(folder, { apiKey: 'vorlyn_from_file' });
      expect(await resolveApiKey(folder, {})).toBe('vorlyn_from_file');
    });

    it('prefers VORLYN_API_KEY over the credentials file', async () => {
      await writeCredentials(folder, { apiKey: 'vorlyn_from_file' });
      expect(await resolveApiKey(folder, { VORLYN_API_KEY: 'vorlyn_from_env' })).toBe(
        'vorlyn_from_env',
      );
    });
  });
