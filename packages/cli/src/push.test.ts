...
describe('runPush', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'mdloop-cli-push-'));
  ...
  describe('partial-run durability', () => {
    ...
    function stubClient(uploadDocument: MdloopMcpClient['uploadDocument']): MdloopMcpClient {
      return {
    ...
    function depsWith(client: MdloopMcpClient): {
      deps: PushDeps;
      exits: number[];
    ...
    it('writes the manifest for uploads that completed before a signal', async () => {
      ...
      expect(err.join('\n')).toMatch(/re-run "mdloop push" to finish/);
      const manifest = await readManifest(folder);
