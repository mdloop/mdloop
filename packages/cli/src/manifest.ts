...
/** Per-file tracking state persisted in `.mdloop/manifest.json`. */
...
/**
 * `.mdloop/manifest.json` — safe to commit. Contains
...
export function manifestPath(folder: string): string {
  return path.join(mdloopDir(folder), MANIFEST_FILENAME);
}

/** Reads `.mdloop/manifest.json`; returns undefined when the folder isn't linked. */
