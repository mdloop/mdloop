...
describe('folder-projects', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-data-'));
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-folder-'));
  });
  ...
  describe('readFolderProjects', () => {
    ...
    it('two different, never-before-touched data directories never see each others entries (no shared "empty" object)', async () => {
      const otherDataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-other-'));
      try {
  ...
  describe('concurrency', () => {
    it('two concurrent recordFolderProject calls for different folders both survive (no lost update)', async () => {
      const folderA = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-a-'));
      const folderB = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-b-'));
      try {
        await Promise.all([
  ...
  describe('listFolderProjects', () => {
    it('returns entries sorted by linkedAt ascending, each annotated with its folder key', async () => {
      const folderA = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-list-a-'));
      const folderB = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-folderprojects-list-b-'));
      try {
        await recordFolderProject(
          dataDir,
          folderB,
