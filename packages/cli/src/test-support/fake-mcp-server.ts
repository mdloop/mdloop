...
function buildFakeServer(
  ...
  const server = new McpServer({ name: 'mdloop-fake', version: '0.0.1' });
 ...
 * package may not import `@mdloop/mcp` (dependency-cruiser `cli-http-only`),
...
export async function startFakeMcpServer(
  ...
  const apiKey = opts.apiKey ?? 'mdloop_test_key';
