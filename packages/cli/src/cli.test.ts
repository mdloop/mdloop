...
describe('cli run()', () => {
  ...
  describe('against a live folder + fake server', () => {
    ...
    beforeEach(async () => {
      folder = await mkdtemp(path.join(tmpdir(), 'mdloop-cli-argv-'));
    ...
    it('installs the git hook by default, and --no-git-hook suppresses it', async () => {
      ...
      expect(await readFile(hook, 'utf8')).toMatch(/Managed by "mdloop link"/);
    ...
    // dispatch has no data-dir injection seam, same reason "mdloop open" is
    ...
    it('rejects an unknown "projects" subcommand', async () => {
      ...
      expect(code).toBe(1);
      expect(err.join('\n')).toMatch(/Unknown "mdloop projects" subcommand/);
    });
    ...
    it('rejects an unknown "serve" subcommand', async () => {
      ...
      expect(code).toBe(1);
      expect(err.join('\n')).toMatch(/Unknown "mdloop serve" subcommand/);
    });
  });
});
