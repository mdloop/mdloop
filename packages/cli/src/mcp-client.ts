...
export interface VorlynMcpClient {
...
/**
 * MCP client wrapper over vorlyn's streamable-HTTP transport (ARCHITECTURE.md
 * §9 — the CLI is just another MCP agent, authenticating the same way any
 * other client would: `Authorization: Bearer vorlyn_...`, stateless, a fresh
 * server per request on vorlyn's side, so no session id is needed here
 * either).
 *
 ...
 */
export async function connectMcpClient(endpoint: string, apiKey: string): Promise<VorlynMcpClient> {
  assertTransportSafe(endpoint);
  const transport = new StreamableHTTPClientTransport(new URL(endpoint), {
    requestInit: { headers: { Authorization: `Bearer ${apiKey}` } },
  });
  const client = new Client({ name: 'vorlyn-cli', version: '0.0.1' });
 ...
 *
 * `.vorlyn/manifest.json` is committed and shared, so its `endpoint` is
