...
/** `.vorlyn/endpoint-trust.json` — local and gitignored, never committed. */
export interface EndpointTrust {
  trustedOrigin: string;
}

export function endpointTrustPath(folder: string): string {
  return path.join(vorlynDir(folder), TRUST_FILENAME);
}
...
function parseEndpoint(endpoint: string): URL {
  try {
    ...
    throw new EndpointRefusedError(
      `Refusing to connect: "${endpoint}" in .vorlyn/manifest.json is not a valid URL.`,
    );
  }
}
...
/**
 * Scheme guard. `.vorlyn/manifest.json` is committed and shared by design, so
...
export function assertTransportSafe(endpoint: string): void {
  ...
  throw new EndpointRefusedError(
    `Refusing to send your API key to ${endpoint} over ${url.protocol.replace(':', '')} — ` +
      ...
      'Check the "endpoint" field in .vorlyn/manifest.json.',
  );
}
 ...
 * make every `vorlyn serve` restart look like a same-severity event as a
...
export async function assertEndpointTrusted(folder: string, endpoint: string): Promise<void> {
  ...
  if (pinned !== undefined && pinned !== origin) {
    throw new EndpointRefusedError(
      `Refusing to connect: the endpoint in .vorlyn/manifest.json changed from ${pinned} to ${origin} since this folder was last used.\n` +
