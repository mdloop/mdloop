...
describe('installGitPostCommitHook', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-hook-'));
  });
  ...
  it('upgrades a stale hook of ours in place', async () => {
    await gitInit(folder);
    await installGitPostCommitHook(folder);
    // An older vorlyn-managed hook: our marker, different body.
    await writeFile(
      hookPath(folder),
      `#!/usr/bin/env bash\n${GIT_HOOK_MARKER}\nvorlyn push --ancient-flag\n`,
      'utf8',
    );
  ...
  it('offers a paste-able one-liner for the foreign-hook case', () => {
    expect(GIT_HOOK_PUSH_LINE).toMatch(/vorlyn/);
    expect(GIT_HOOK_PUSH_LINE).toMatch(/push --note/);
    expect(GIT_HOOK_PUSH_LINE).toMatch(/VORLYN_CLI_PATH/);
...
describe('uninstallGitPostCommitHook', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-unhook-'));
  });
  ...
  it('removes a vorlyn-managed hook and returns removed', async () => {
    await gitInit(folder);
    await installGitPostCommitHook(folder);
...
describe('gitHookStatus', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-hookstatus-'));
  });
  ...
  it('returns vorlyn_managed after installGitPostCommitHook', async () => {
    await gitInit(folder);
    await installGitPostCommitHook(folder);
    expect(await gitHookStatus(folder)).toBe('vorlyn_managed');
  });
...
describe('resolveGitRoot', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-root-'));
  });
