...
export interface OpenOptions {
   ...
   * remote/CI user of a *published* `mdloop` needs too, not just a test
...
export interface OpenDeps {
   ...
   *  `mdloop serve` can share exactly the same implementation. Overridable in tests via
...
/**
 * `mdloop open ./folder` — the zero-install local experience: link-if-needed
 ...
 *   serving other folders (`mdloop serve`, or another `mdloop open`).
 ...
 *   that existed before `mdloop serve` did.
 *
...
export async function runOpen(
  ...
  async function cleanup(): Promise<void> {
    ...
    // start; other folders (or a `mdloop serve` daemon) may depend on it.
  ...
  try {
    ...
    if (existing) {
      io.println(
        `Attaching to the already-running local mdloop instance (pid ${String(existing.pid)})...`,
      );
      if (!existing.rootUrl || !existing.mcpEndpoint) {
        io.errln('The running instance has no recorded URL — try "mdloop serve stop" then retry.');
        return 1;
      }
      rootUrl = existing.rootUrl;
      mcpEndpoint = existing.mcpEndpoint;
    } else {
      io.println(`Starting local mdloop instance (data: ${dataDir})...`);
      ...
      // real teardown against the way the pre-`mdloop serve` version of
      ...
      if (raced.kind === 'error') {
        io.errln(`Could not start the local mdloop instance: ${(raced.error as Error).message}`);
        return 1;
      }
    ...
    if (!manifest) {
      ...
      // Missing when `existingKey` is already set (e.g. MDLOOP_API_KEY set
      ...
      // (`credentials.ts`) is the same fallback `mdloop link --autoProvision`
      // now also reaches for on its own against a local endpoint.
      if (!existingKey) {
        const bootstrapKey = await readLocalBootstrapApiKey(dataDir);
        if (!bootstrapKey) {
          io.errln(
            'No admin API key found for this local instance — the embedded server should have minted one on first boot. Try deleting the data directory and running "mdloop open" again.',
          );
          await cleanup();
          return 1;
        }
      ...
      io.errln(
        `${options.folder} is already linked to ${manifest.endpoint}, which is not a local endpoint. ` +
          '"mdloop open" only works with a local embedded instance — run "mdloop unlink" here first if you want to relink locally.',
      );
      await cleanup();
      return 1;
    }
    ...
    // instance — the user can re-run "mdloop push" once they've resolved
    // whatever it flagged.
    ...
    if (ownedInstance) {
      io.println(`mdloop is running at ${rootUrl} — Ctrl-C to stop.`);
    } else {
      io.println(
        `mdloop is running at ${rootUrl}. It was already running in the background — Ctrl-C here only stops this command, not the server ("mdloop serve stop" does that).`,
      );
    }
    ...
    if (ownedInstance) {
      io.println('Stopping local mdloop instance...');
    } else {
      io.println(
        'The local mdloop server is still running in the background — "mdloop serve stop" to stop it.',
      );
    }
    await cleanup();
    return 0;
  } catch (error) {
    io.errln(`mdloop open failed: ${(error as Error).message}`);
    await cleanup();
    return 1;
  } finally {
