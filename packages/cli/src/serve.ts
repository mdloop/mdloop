 ...
 * entrypoint rather than `@mdloop/api`'s or `@mdloop/mcp`'s: `mdloop
...
/**
 * `mdloop serve start` — the launcher half. Idempotent (returns promptly
 ...
 * returning, so a caller (a human, or `mdloop-ensure.sh`) never has to
 * poll separately.
 */
async function runServeStart(io: Io, deps: ServeDeps): Promise<number> {
  ...
  if (existing) {
    io.println(
      `mdloop is already running at ${existing.rootUrl ?? '(unknown)'} (pid ${String(existing.pid)}).`,
    );
    return 0;
  }
  ...
  try {
    supervisor = deps.spawnSupervisor(fd.fd, { ...process.env, MDLOOP_DATA_DIR: dataDir });
  } finally {
    await fd.close();
  }
  ...
  if (record?.state !== 'running') {
    ...
    io.errln(
      supervisorDied
        ? 'The mdloop server process exited before it finished starting.'
        : 'Timed out waiting for the local mdloop server to start.',
    );
  ...
  io.println(
    `mdloop is running at ${record.rootUrl ?? '(unknown)'} (pid ${String(record.pid)}). ` +
      `Logs: ${logFile}. Stop it with "mdloop serve stop".`,
  );
  return 0;
}
...
async function runServeForeground(io: Io, deps: ServeDeps): Promise<number> {
  ...
  try {
    ...
    if (error instanceof InstanceConflictError) {
      io.println(`mdloop is already running (pid ${String(error.existing.pid)}).`);
      return 0;
    }
    throw error;
  }
  ...
  try {
    ...
    io.println(`mdloop serve: running at ${instance.rootUrl} (pid ${String(process.pid)}).`);

    await interrupted;

    io.println('mdloop serve: stopping...');
    await instance.stop();
    await releaseInstanceRecord(dataDir, record);
    return 0;
  } catch (error) {
    await releaseInstanceRecord(dataDir, record);
    io.errln(`mdloop serve failed: ${(error as Error).message}`);
...
/**
 * `mdloop serve stop`. Idempotent when nothing is running. Refuses to stop
 * a `mdloop open` foreground session from here — SIGTERM would work (its
...
async function runServeStop(io: Io, deps: ServeDeps): Promise<number> {
  ...
  if (!record || !deps.isAlive(record.pid)) {
    io.println('The local mdloop server is not running.');
    return 0;
  }
  if (record.owner === 'open') {
    io.errln(
      `That instance is a foreground "mdloop open" session (pid ${String(record.pid)}) — press Ctrl-C in the terminal running it.`,
    );
    return 1;
  }
...
/** `mdloop serve status`. Exit 0 running / 1 not — `systemctl is-active` shaped, deliberately: it's
 *  what makes `mdloop serve status || mdloop serve start` a correct one-liner for a hook to use. */
...
/** `mdloop serve start|stop|status [--foreground] [--json]` — dispatched from `cli.ts`. */
export async function runServe(
  ...
  switch (subcommand) {
    ...
    default:
      io.errln(
        `Unknown "mdloop serve" subcommand: ${subcommand ?? '(none)'}. Use start, stop, or status.`,
      );
      return 1;
  }
}
