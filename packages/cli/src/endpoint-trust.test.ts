...
describe('endpoint trust', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-trust-'));
  });
  ...
  describe('transport guard', () => {
    it('allows https to any host', () => {
      expect(() => {
        assertTransportSafe('https://vorlyn.example.com/mcp');
      }).not.toThrow();
    });
  ...
  describe('origin', () => {
    it('is scheme + host + port, dropping the path', () => {
      expect(endpointOrigin('https://vorlyn.example.com:8443/mcp')).toBe(
        'https://vorlyn.example.com:8443',
      );
    });
  ...
  describe('trust on first use', () => {
    it('trusts and pins the first endpoint a folder ever sees', async () => {
      await expect(
        assertEndpointTrusted(folder, 'https://vorlyn.example.com/mcp'),
      ).resolves.toBeUndefined();
      expect(await readTrustedOrigin(folder)).toBeUndefined();

      await pinTrustedOrigin(folder, endpointOrigin('https://vorlyn.example.com/mcp'));
    ...
    it('allows the unchanged origin repeatedly, including a different path', async () => {
      ...
      await expect(
        assertEndpointTrusted(folder, 'https://vorlyn.example.com/other/path'),
      ).resolves.toBeUndefined();
    });
    ...
    it('refuses a silently-changed origin, naming both sides and the way out', async () => {
      ...
      await expect(
        ...
        /changed from https:\/\/vorlyn\.example\.com to https:\/\/attacker\.example\.net/,
      );
      await expect(
        assertEndpointTrusted(folder, 'https://attacker.example.net/mcp'),
    ...
    it('still refuses a same-host, different-port move for a remote endpoint', async () => {
      await pinTrustedOrigin(folder, 'https://vorlyn.example.com:8443');
      await expect(
        assertEndpointTrusted(folder, 'https://vorlyn.example.com:9999/mcp'),
      ).rejects.toBeInstanceOf(EndpointRefusedError);
    });
