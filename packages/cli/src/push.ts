...
import type { MdloopMcpClient } from './mcp-client.js';
...
export interface PushDeps {
  ...
  connect: (folder: string, endpoint: string, apiKey: string) => Promise<MdloopMcpClient>;
  /**
...
const PATH_FAILURE_MESSAGES: Record<string, string> = {
  path_taken:
    'Another document already exists at this path in the project — resolve the conflict in mdloop or move one of them.',
  invalid_path:
    "This file's path isn't valid for mdloop (check for empty segments, `.`/`..`, or unusual characters).",
};
...
/**
 * `mdloop push` — one-shot sync of local `*.md` files to mdloop document
...
export async function runPush(
  ...
  if (!apiKey) {
    io.errln(
      'No API key found. Set MDLOOP_API_KEY or create .mdloop/credentials in the linked folder.',
    );
    return 1;
  }
  ...
  if (state.interrupted) {
    io.errln(
      'Interrupted. Completed uploads are recorded in .mdloop/manifest.json — re-run "mdloop push" to finish.',
    );
    deps.exit(1);
    return 1;
  }
