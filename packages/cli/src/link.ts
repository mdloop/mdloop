...
import { ensureVorlynConfig, readVorlynConfig } from './vorlyn-config.js';
...
export interface LinkOptions {
  ...
  endpoint?: string;
  /**
   * Trigger recorded in `.vorlyn/config.json`. Defaults to `'commit'` (the
   * harness-agnostic git hook); the caller in `cli.ts` maps `--trigger`.
   * Ignored when the repo already has a config — see `ensureVorlynConfig`.
   */
  trigger?: Trigger;
  /** `false` for `vorlyn link --no-git-hook`. Defaults to installing. */
...
/**
 * `vorlyn link` — binds a local folder to an existing Vorlyn project.
...
export async function runLink(
  ...
  await ensureVorlynDir(options.folder);

  const dataDir = options.dataDir ?? resolveDataDir();
  const existing = await readManifest(options.folder);
  let endpoint = options.endpoint ?? existing?.endpoint ?? process.env.VORLYN_MCP_URL;
  if (!endpoint) {
    ...
    // `vorlyn link` (this is exactly what `vorlyn-ensure.sh` and a human
  ...
  if (!apiKey && options.autoProvision && isLocalEndpoint(endpoint)) {
    // Same bootstrap key `vorlyn open` reads to complete its own first
    ...
    // a folder that was never `vorlyn open`ed by hand can never link at
    // all — exactly the case `vorlyn-ensure.sh`'s bare `vorlyn link` hits
  ...
  if (!apiKey) {
    io.errln(
      'No API key found. Set VORLYN_API_KEY or create .vorlyn/credentials in the linked folder — link does not create one.',
    );
    return 1;
  }
  ...
  try {
    ...
    // see the Fix 1 writeup. "vorlyn unlink" then a fresh link is the only
    // sanctioned way to switch a folder to a different project.
    if (existing && existing.projectId !== projectId) {
      io.errln(
        `This folder is already linked to project ${existing.projectId}. A folder links to one project at a time — run "vorlyn unlink" here first, then "vorlyn link --project ${projectId}" to switch.`,
      );
      return 1;
    }
    ...
    await ensureVorlynConfig(options.folder, options.trigger ?? 'commit');
    // Read back the *resolved* trigger rather than trusting `options.trigger`:
    // on a relink where config.json already exists, `ensureVorlynConfig` is a
    // no-op and the options-level value can be stale or simply wrong — the
    // file on disk is the source of truth.
    const resolvedTrigger = (await readVorlynConfig(options.folder))?.trigger ?? 'commit';
...
async function reportGitHookInstall(folder: string, trigger: Trigger, io: Io): Promise<void> {
  ...
  switch (result) {
    case 'installed':
      io.println(
        'Installed a git post-commit hook — every commit will now push tracked files to Vorlyn automatically.',
      );
      io.println(
      ...
      if (trigger === 'commit') {
        io.println(
          'Not inside a git repo yet, so the commit-trigger hook was not installed. Once this ' +
            'becomes a git repo, re-run "vorlyn link" to install it — until then, nothing pushes ' +
            'automatically except a manual "vorlyn push" or the agent pushing proactively.',
        );
      }
      break;
  }
