...
describe('startLocalInstance', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-local-instance-'));
  });
   ...
   * (`vorlyn serve start` against an occupied port left the detached
...
describe('embeddedEntrypointCandidates / resolveEmbeddedEntrypointFrom', () => {
  ...
  it('prefers the installed (bundled) layout — a sibling dist/<pkg>/ next to its own dist/cli/', async () => {
    tmpRoot = await mkdtemp(path.join(tmpdir(), 'vorlyn-entrypoint-'));
  ...
  it('throws naming every candidate tried when none exist', async () => {
    ...
    await expect(resolveEmbeddedEntrypointFrom(fromUrl, 'api')).rejects.toThrow(
      /Could not find @vorlyn\/api's embedded entrypoint/,
    );
  });
});
...
describe('stopChild', () => {
   ...
   * `vorlyn serve start` against an occupied port: the api child crashed
