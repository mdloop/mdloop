...
import { vorlynDir } from './vorlyn-dir.js';
...
export interface UnlinkOptions {
  folder: string;
  /** `false` for `vorlyn unlink --no-git-hook`. Defaults to removing a vorlyn-managed hook. */
  removeGitHook?: boolean;
}

/**
 * `vorlyn unlink` — reverses `vorlyn link`: deletes
 * `.vorlyn/` (manifest, config, credentials, lock, endpoint trust pin, the
 ...
 * "install" — leaving a hook behind that still fires `vorlyn push` against a
...
export async function runUnlink(options: UnlinkOptions, io: Io): Promise<number> {
  ...
  try {
    ...
    await rm(vorlynDir(options.folder), { recursive: true, force: true });
...
async function reportGitHookUninstall(folder: string, io: Io): Promise<void> {
  ...
  switch (result) {
    case 'removed':
      io.println('Removed the git post-commit hook — commits will no longer push to Vorlyn.');
      break;
    case 'foreign':
      io.println(
        'A git post-commit hook exists but was not installed by "vorlyn link" — left untouched.',
      );
      break;
    case 'not_present':
    case 'not_a_git_repo':
