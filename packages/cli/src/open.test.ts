...
describe('runOpen', () => {
  ...
  beforeEach(async () => {
    dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-open-data-'));
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-open-folder-'));
  ...
  describe('own mode (nothing already running)', () => {
    it('happy path: fresh folder links, pushes, opens the browser, and tears down on signal', async () => {
      ...
      expect(out.some((l) => l.includes('Vorlyn is running at'))).toBe(true);
    ...
    it('--no-browser skips deps.openBrowser entirely, but still reaches the running-state prompt', async () => {
      ...
      // same "Vorlyn is running at" println the browser-opening path prints
      // right after (or, here, instead of) opening it.
      await waitUntil(() => out.some((l) => l.includes('Vorlyn is running at')));
      signals.emit('SIGINT');
    ...
    it('already linked to a non-local endpoint: refuses without linking or pushing', async () => {
      const manifest: Manifest = {
        endpoint: 'https://vorlyn.example.com/mcp',
        projectId: 'proj_1',
        files: {},
      };
    ...
    it('startLocalInstance failure: returns 1, and the error is reported', async () => {
      ...
      expect(code).toBe(1);
      expect(err.some((l) => l.includes('Could not start the local Vorlyn instance'))).toBe(true);
    ...
    it('missing admin API key with no other credentials: clear error, cleans up, returns 1', async () => {
      // No writeBootstrapFile() call, no .vorlyn/credentials, no VORLYN_API_KEY.
...
describe('attach mode keeps the process alive until signaled (real subprocess)', () => {
  it('does not exit on its own, and exits 0 once signaled', async () => {
    ...
    const scriptDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-open-keepalive-'));
