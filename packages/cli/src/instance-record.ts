...
/**
 * `<dataDir>/instance.json` — the one embedded mdloop instance a machine's
 * data directory can have running at a time. `owner` distinguishes a
 * `mdloop serve` daemon (detached, survives the launching terminal closing)
 * from a `mdloop open` foreground session (tied to that terminal, torn down
 * on its own Ctrl-C) — see `serve.ts` and `open.ts`'s attach/own-mode split.
 * `logFile` is present only for `owner: "serve"`; a `mdloop open` session
...
export class InstanceConflictError extends Error {
  constructor(public readonly existing: InstanceRecord) {
    super(
      `another mdloop instance is running (pid ${String(existing.pid)}, owner ${existing.owner})`,
    );
 ...
 * `mdloop-ensure.sh`'s `serve status`. A record only counts when its pid is
