...
export function lockPath(folder: string): string {
  return path.join(mdloopDir(folder), '.lock');
}

export class LockConflictError extends Error {
  constructor(pid: number) {
    super(`another mdloop process is running (pid ${String(pid)})`);
...
/**
 * Acquires `.mdloop/.lock`, protecting one `push` from racing another.
...
/**
 * Removes `.mdloop/.lock`, but only when it is still the one `owned` describes.
