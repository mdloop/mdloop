...
import { vorlynGitignorePath } from './vorlyn-dir.js';
...
describe('runLink', () => {
  ...
  beforeEach(async () => {
    folder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-'));
  ...
  it('errors clearly with no API key available', async () => {
    const bareFolder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-bare-'));
  ...
  it('writes .vorlyn/.gitignore so the credentials file can never be committed', async () => {
    const { io } = collectIo();
    await runLink({ folder, endpoint: fake.url, projectId: 'proj_1' }, io);
    const ignore = await readFile(vorlynGitignorePath(folder), 'utf8');
    expect(ignore).toMatch(/^credentials$/m);
    expect(ignore).toMatch(/^\.lock$/m);
    expect(ignore).toMatch(/^endpoint-trust\.json$/m);
  });

  it('writes .vorlyn/.gitignore even when link bails out for want of a key', async () => {
    const bareFolder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-bare-'));
    const { io } = collectIo();
    ...
    expect(await readFile(vorlynGitignorePath(bareFolder), 'utf8')).toMatch(/^credentials$/m);
    await rm(bareFolder, { recursive: true, force: true });
  });
  ...
  // .vorlyn/config.json records which event owns the push. Seeded on the first
  ...
  describe('git post-commit hook', () => {
    ...
    it('never touches a foreign hook, and prints the line to add instead', async () => {
      ...
      expect(printed).toContain('"$VORLYN" push --note');
    });
  ...
  it('warns about the missing hook when linking outside git with the default trigger', async () => {
    ...
    expect(out.join('\n')).toMatch(/re-run "vorlyn link" to install it/);
  });
  ...
  describe('endpoint resolution with no explicit endpoint', () => {
    ...
    beforeEach(async () => {
      dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-endpoint-datadir-'));
    });
     ...
     * manually against a real running daemon, where a bare `vorlyn link`
     * (no `--endpoint`, no existing manifest, no `VORLYN_MCP_URL`) failed
     * to connect outright because the real daemon was never on port 3001 in
     * the first place. This is exactly the call shape `vorlyn-ensure.sh`
  ...
  describe('autoProvision', () => {
    ...
    beforeEach(async () => {
      dataDir = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-datadir-'));
    });
    ...
    /**
     * Regression pin: `vorlyn link` never creates an API key of its own —
     * true and unchanged for the explicit/remote case — but a brand-new
     * folder that was never `vorlyn open`ed by hand (this is exactly what
     * `vorlyn-ensure.sh`'s bare `vorlyn link` hits for a fresh repo) used
     * to have no way to get one at all under auto-provisioning, since only
     * `vorlyn open`'s bootstrap logic ever read `bootstrap.json`. Against a
     * local endpoint, with auto-provisioning on, this must now succeed using
     * the same bootstrap key `vorlyn open` reads — and persist it, so a
    ...
    it('falls back to the local bootstrap API key when nothing else has one, against a local endpoint with autoProvision on', async () => {
      const freshFolder = await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-nocreds-'));
      await writeFile(
      ...
      try {
        ...
        // Persisted for next time, same as vorlyn open's own bootstrap flow.
        const creds = JSON.parse(
          await readFile(path.join(freshFolder, '.vorlyn', 'credentials'), 'utf8'),
        ) as { apiKey: string };
    ...
    it('reuses the mapping on a later link of the same (unlinked-in-between) folder, without creating again', async () => {
      ...
      // Remove only the manifest (as "vorlyn unlink" would) — the
      // machine-wide projects.json mapping in `dataDir` is untouched.
      await rm(path.join(folder, '.vorlyn', 'manifest.json'));
    ...
    it('reuses an existing project whose name matches the folder, without creating one', async () => {
      const namedFolder = path.join(
        await mkdtemp(path.join(tmpdir(), 'vorlyn-cli-link-parent-')),
        'Alpha',
      );
