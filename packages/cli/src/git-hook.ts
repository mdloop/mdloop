 ...
 *
 * So the canonical post-commit script lives here, in the CLI, and `mdloop link`
 ...
 * in the first place.
 */
export const GIT_HOOK_MARKER = '# Managed by "mdloop link" — safe to regenerate; do not hand-edit.';
 ...
 *
 * `command -v mdloop` covers a global npm install; `MDLOOP_CLI_PATH` is the
...
export const GIT_HOOK_PUSH_LINE =
  'MDLOOP=$(command -v mdloop || printf \'%s\' "${MDLOOP_CLI_PATH:-}"); ' +
  '[ -x "$MDLOOP" ] && "$MDLOOP" push --note "$(git log -1 --pretty=%s)" --quiet || true';
...
${GIT_HOOK_MARKER}
#
# mdloop sync — pushes tracked markdown files to mdloop after every commit.
# Installed by \`mdloop link\`; skip it with \`mdloop link --no-git-hook\`, and
...
# Locate the CLI. MDLOOP_CLI_PATH is the escape hatch for a monorepo build that
...
# of what changed, which is exactly what mdloop's Compare surface shows the
# reviewer. Write commit subjects accordingly.
cd "$REPO_ROOT" || exit 0
"$MDLOOP" push --note "$(git log -1 --pretty=%s)" --quiet
...
function isMdloopManagedHook(content: string): boolean {
 ...
 * same non-clobbering philosophy as `ensureMdloopGitignore` and the endpoint
 ...
 *   current template. This is what makes re-running `mdloop link` upgrade the
...
export async function installGitPostCommitHook(folder: string): Promise<GitHookInstallResult> {
  ...
  if (!isMdloopManagedHook(current)) return 'foreign';
...
export type GitHookStatus = 'not_a_git_repo' | 'missing' | 'foreign' | 'mdloop_managed';
 ...
 * means there was never any git for `mdloop link`'s own hook-install step to
...
export async function gitHookStatus(folder: string): Promise<GitHookStatus> {
  ...
  return isMdloopManagedHook(current) ? 'mdloop_managed' : 'foreign';
}

export type GitHookUninstallResult = 'removed' | 'foreign' | 'not_present' | 'not_a_git_repo';

/**
 * Removes `.git/hooks/post-commit`, the counterpart `mdloop unlink` calls.
 *
