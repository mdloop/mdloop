...
import { Pool } from 'pg';
import type { ApiKeyRepository, Actor, AuthPort, AuthProfile, EmailPort } from '@mdloop/app';
import { createApiKey } from '@mdloop/app';
...
/**
 * Embedded-mode composition root — the process `mdloop open ./folder`
 ...
 *      this process (never a human-edited .env) — `MDLOOP_AUTH_MODE` is
 ...
 * `mdloop_login` role reachable over its wire protocol the way real
 ...
 * mdloop_app`/etc. before every tenant query regardless of which identity is
...
// so a document link stays valid across a `mdloop serve` restart. Only a
...
await setupTelemetryIfAvailable('mdloop-api-selfhost-embedded');
...
// MDLOOP_AUTH_MODE=single-user (or leaves it unset, same default). Any other
...
if (authMode !== 'single-user') {
  throw new Error(
    `Embedded mode only supports MDLOOP_AUTH_MODE=single-user (got ${JSON.stringify(authMode)}) — oidc requires a real browser-reachable OIDC provider, which does not fit "mdloop open ./folder"'s single local user.`,
  );
}
...
if (!existingOrgId) {
  ...
  if (!admin) {
    throw new Error(
      'Embedded instance already has an organization but no user matching MDLOOP_ADMIN_EMAIL — the admin identity must not change across restarts of the same data directory.',
    );
  }
  adminUserId = admin.id;
...
/**
 * Mints the one API key `mdloop open`'s "link-if-needed" step
 ...
 * minted fresh per run. `MDLOOP_DATA_DIR` is set by the CLI on every spawn;
 ...
 * resolution.ts`): the first `mdloop link`/`mdloop open` for a given
...
if (!dataDir) {
  throw new Error('MDLOOP_DATA_DIR must be set — this entrypoint is only ever spawned by the CLI');
}
const bootstrapPath = path.join(dataDir, 'bootstrap.json');
if (!existingOrgId) {
  const actor: Actor = { ctx: { orgId: adminOrgId, userId: adminUserId }, role: 'admin' };
  const minted = await createApiKey(apiKeys, actor, 'mdloop open');
  if (!minted.ok) {
...
server
  ...
  .then(async () => {
    ...
    // see the matching comment on @mdloop/mcp's embedded main.
