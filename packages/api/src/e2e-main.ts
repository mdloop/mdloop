...
import { NoopSeatSync, signIn, uploadNewDocument } from '@mdloop/app';
import { NoopTelemetry, UnlimitedRateLimiter } from '@mdloop/app/test-support';
import type { DocumentId, ProjectId } from '@mdloop/shared';
import {
  FsStorage,
...
const blobDir = await mkdtemp(path.join(tmpdir(), 'mdloop-e2e-blobs-'));
...
// below (`wos_code-e2e` / e2e@mdloop.test) — the later browser-driven sign-in
...
const mentionBuddy = await directory.createUser(e2eBootstrap.value.orgId, {
  ...
  email: 'mention-buddy@mdloop.test',
