...
import { createGuestShare, redeemGuestShare, requireDocumentAccess } from '@mdloop/app';
import type { DocumentId, OrgId, UserId } from '@mdloop/shared';
...
export function registerGuestRedeemRoute(
  ...
  server.post<{ Body: { token: string } }>(
    ...
    async (req, reply) => {
      ...
      // Overwriting `mdloop_session` here would strand them in guest mode
