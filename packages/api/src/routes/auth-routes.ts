...
import type { AuthPort, DirectoryRepository, SeatSyncPort } from '@vorlyn/app';
import { signIn } from '@vorlyn/app';
...
export function registerAuthRoutes(
  ...
  server.get<{ Querystring: { invite?: string } }>('/auth/login', (req, reply) => {
    ...
    void reply.setCookie('vorlyn_oauth_state', state, {
      httpOnly: true,
    ...
    if (req.query.invite) {
      void reply.setCookie('vorlyn_invite_token', req.query.invite, {
        httpOnly: true,
        secure: config.secureCookies,
        sameSite: 'lax',
        path: '/api/auth',
  ...
  server.get<{ Querystring: { code?: string; state?: string } }>(
    ...
    async (req, reply) => {
      ...
      const expectedState = req.cookies.vorlyn_oauth_state;
      if (!code || !state || !expectedState || state !== expectedState) {
        return reply.code(400).send({ error: 'invalid_oauth_state' });
      }
      void reply.clearCookie('vorlyn_oauth_state', { path: '/api/auth' });
      const inviteToken = req.cookies.vorlyn_invite_token;
      if (inviteToken) void reply.clearCookie('vorlyn_invite_token', { path: '/api/auth' });
