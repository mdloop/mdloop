...
/**
 * SMTP-backed email adapter for self-hosted mdloop instances (open-source
...
export class SmtpEmailAdapter implements EmailPort {
  ...
  async sendOrgInvite(
    ...
    await this.transporter.sendMail({
      from: this.from,
      to,
      subject: `You've been invited to join ${orgName} on mdloop`,
      text: [
        `You've been invited to join "${orgName}" on mdloop.`,
  ...
  async sendGuestShare(input: {
    ...
    await this.transporter.sendMail({
      from: this.from,
      to: input.to,
      subject: `"${input.documentTitle}" was shared with you on mdloop`,
      text: [
        `"${input.documentTitle}" was shared with you on mdloop.`,
        `View it: ${input.url}`,
