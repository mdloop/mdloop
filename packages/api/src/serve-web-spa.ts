...
/**
 * Gated so an operator fronting Vorlyn with their own reverse proxy/static
...
export function webSpaServingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (env.VORLYN_SERVE_WEB ?? 'true').trim() !== 'false';
}
 ...
 *
 * Loud on purpose: a Vorlyn that answers `/readyz` while serving no web app
...
export async function resolveWebDist(
  ...
  return firstExistingRelative(
    candidates,
    'a built web SPA (see "pnpm build", or set VORLYN_SERVE_WEB=false if a reverse proxy or ' +
      'static host serves the web UI instead — "pnpm typecheck"/"tsc --build" alone only emits ' +
      '.d.ts files for @vorlyn/web, never the actual "vite build" output)',
