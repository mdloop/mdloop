import { Pool } from 'pg';
import type { AuthPort } from '@mdloop/app';
import {
  FsStorage,
  OtelTelemetry,
...
const dbName = process.env.DEV_DB_NAME ?? 'mdloop_dev';
const adminUrl =
...
const server = await buildServer({
  config: {
    ...
    webAppUrl,
    webOrigin: webAppUrl,
    sessionSecret: 'mdloop-local-dev-secret-do-not-deploy-me',
    secureCookies: false,
  },
  auth: loopbackAuth,
  ...
  storage: new FsStorage(process.env.BLOB_STORAGE_DIR ?? '.mdloop-blobs'),
...
// eslint-disable-next-line no-console
console.log(`mdloop api (dev, loopback auth) on ${baseUrl} — db ${dbName}`);
