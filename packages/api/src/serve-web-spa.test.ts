...
describe('webSpaServingEnabled', () => {
  it('defaults to serving when VORLYN_SERVE_WEB is unset', () => {
    expect(webSpaServingEnabled({})).toBe(true);
  });

  it('serves for any value other than the literal "false"', () => {
    expect(webSpaServingEnabled({ VORLYN_SERVE_WEB: 'true' })).toBe(true);
    expect(webSpaServingEnabled({ VORLYN_SERVE_WEB: '' })).toBe(true);
    expect(webSpaServingEnabled({ VORLYN_SERVE_WEB: 'no' })).toBe(true);
  });

  it('stands down on "false", tolerating surrounding whitespace', () => {
    expect(webSpaServingEnabled({ VORLYN_SERVE_WEB: 'false' })).toBe(false);
    expect(webSpaServingEnabled({ VORLYN_SERVE_WEB: '  false  ' })).toBe(false);
  });
});
...
describe('resolveWebDist', () => {
  ...
  it('names every candidate tried and how to fix it, when none are built', async () => {
    ...
    ).rejects.toThrow(/pnpm build.*VORLYN_SERVE_WEB=false.*\/some\/where\/web\/dist.*\/dist\/web/s);
  });
});
