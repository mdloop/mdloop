...
import type { AuthPort, TelemetryPort } from '@mdloop/app';
 ...
 * test-harness equivalent of the browser SPA auto-echoing `mdloop_csrf`. Requests
...
export async function createTestServer(
  ...
  const blobDir = await mkdtemp(path.join(tmpdir(), 'mdloop-api-blobs-'));
...
export async function loginAs(
  ...
  const inviteCookie = login.cookies.find((c) => c.name === 'mdloop_invite_token');
  ...
  const callback = await server.inject({
    ...
    cookies: {
      mdloop_oauth_state: stateCookie.value,
      ...(inviteCookie ? { mdloop_invite_token: inviteCookie.value } : {}),
    },
  });
