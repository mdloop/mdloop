import { Pool } from 'pg';
import type { AuthPort, AuthProfile, DirectoryRepository, EmailPort } from '@vorlyn/app';
import type { Organization, OrgInvite, ShareGrant, User } from '@vorlyn/domain';
import type { InviteId, OrgId } from '@vorlyn/shared';
import {
  OtelTelemetry,
 ...
 * VORLYN_AUTH_MODE — see "Auth adapter selection" below.
 *
...
setupTelemetry('vorlyn-api-selfhost');
...
// (migration 0024's vorlyn_login role), not something a single-tenant
...
// ---- Auth adapter selection (VORLYN_AUTH_MODE, default 'single-user') ----
...
if (authMode === 'single-user') {
  ...
  if (!issuer || !clientId) {
    throw new Error(
      'VORLYN_AUTH_MODE=oidc requires OIDC_ISSUER and OIDC_CLIENT_ID to be set (OIDC_CLIENT_SECRET is optional, for a public/PKCE-only client)',
    );
  }
  ...
  throw new Error(
    `VORLYN_AUTH_MODE must be "single-user" or "oidc" (got ${JSON.stringify(authMode)})`,
  );
}
...
// mechanism, the VORLYN_SERVE_WEB escape hatch, and for why a missing build
