 ...
 * "in mdloop", which would be noise, not signal. Reuses
...
export function SourceBadge({ path }: { path: string | null }): JSX.Element | null {
  ...
  return (
    <span
      ...
      title="Linked to a local folder — mdloop push mirrors this document's structure"
    >
      <IconLink size={11} />
...
/** True for any repo-backed document — `mdloop push` (or the auto-installed
...
export function DocumentList({
   ...
   *  (same `tree-row--folder`/`tree-mdloop`/`lane-signal`/`tree-children`
    ...
    <>
      {pendingRepoAction && (
        <ConfirmDialog
          ...
          body="This document is linked to a local folder. The local file won't be touched, and the repo stays the source of truth — mdloop push still mirrors it here the same way next time."
