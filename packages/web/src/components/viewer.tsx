...
/** Mirrors MAX_COMMENT_BODY_LENGTH in @mdloop/domain — web has no dependency
 * on the domain package, so the cap is duplicated here rather than imported. */
const MAX_COMMENT_LENGTH = 2_000;
/** Mirrors MAX_PROPOSED_TEXT_LENGTH in @mdloop/domain, same reason as above. */
...
const draftKey = (documentId: string): string => `mdloop:draft:${documentId}`;
    ...
    <div className="viewer">
      {uploadCandidate && (
        <ConfirmDialog
          ...
          body={
            document?.path != null
              ? "This document is linked to a local folder. Uploading here replaces what's currently on screen — and the next mdloop push from that folder will silently overwrite it again, since the repo stays the source of truth."
