 ...
 * that has never run `mdloop link` has no way to learn the capability exists.
 *
...
function CliLinkHint({
  ...
  const command = projectId ? `mdloop link --project ${projectId}` : 'mdloop link';
...
function CliLinkedIndicator(): JSX.Element {
  return (
    <aside
      ...
      title="mdloop push (or an auto-installed git hook) mirrors this project's local folder here"
    >
      <IconLink size={13} />
