...
beforeEach(() => {
  ...
  fetchMock.mockReset();
  document.cookie = 'vorlyn_csrf=csrf-token-value';
});
...
describe('api.createApiKey', () => {
  it('POSTs the name with the CSRF echo and returns the once-only key', async () => {
    ...
    fetchMock.mockResolvedValue(jsonResponse({ key: 'vorlyn_secret', record }, 201));

    await expect(api.createApiKey('CI')).resolves.toEqual({ key: 'vorlyn_secret', record });
    const call = callInit();
    expect(call.url).toBe('/api/api-keys');
