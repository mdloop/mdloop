...
import type { Actor, AuthPort } from '@mdloop/app';
import { FakeEmail, NoopTelemetry } from '@mdloop/app/test-support';
import { buildServer } from '@mdloop/api';
import { csrfTokenForSession } from '@mdloop/api/test-support';
import { buildMcpServer } from '@mdloop/mcp';
import type { McpDeps } from '@mdloop/mcp';
import {
  FsStorage,
...
describe('API / MCP parity', () => {
  ...
  function csrfFor(): { cookies: Record<string, string>; headers: Record<string, string> } {
    ...
    return {
      cookies: { mdloop_session: sessionCookie, mdloop_csrf: csrf },
  ...
  beforeAll(async () => {
    ...
    const storage = new FsStorage(await mkdtemp(path.join(tmpdir(), 'mdloop-parity-blobs-')));
    ...
    const stateCookie = login.cookies.find((c) => c.name === 'mdloop_oauth_state');
    ...
    const callback = await server.inject({
      ...
      cookies: { mdloop_oauth_state: stateCookie.value },
    });
    const session = callback.cookies.find((c) => c.name === 'mdloop_session');
  ...
  it('a comment created over MCP is visible over HTTP, and vice versa', async () => {
    ...
    const httpThreads = await server.inject({
      ...
      cookies: { mdloop_session: sessionCookie },
    });
