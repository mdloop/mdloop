# Vorlyn — local development entry points.
#
#   make dev            run the app (API :3000 + web :5173 + MCP :3001) with
#                       loopback auth, persistent local Postgres database "vorlyn_dev"
...
DEV_DB_URL := postgres://vorlyn_login@localhost:5432/vorlyn_dev
...
# api creates/migrates vorlyn_dev on startup (see dev-main.ts); mcp connects
...
dev: build
	...
	pnpm --filter @vorlyn/web dev & web=$$!; \
...
web:
	pnpm --filter @vorlyn/web dev

# API dev-main creates/migrates vorlyn_dev on startup; run `make api` once
