...
```sh
git clone https://github.com/imjasdeepk/vorlyn
cd vorlyn
pnpm install
...
`pnpm typecheck && pnpm --filter @vorlyn/web build` (the `vite build` step) —
...
browser (`vorlyn open` prints the URL). The api/mcp ports default to a fixed pair in the private/
...
document link stays valid across a `vorlyn serve` restart instead of moving with each one. Set
...
Multiple folders can share one instance: `vorlyn open` attaches to an already-running one instead
of starting a second, and `vorlyn serve start` runs the same embedded instance detached — it
survives closing the terminal that started it, so it doesn't have to be exactly one folder at a
time anymore. `vorlyn serve stop`/`vorlyn serve status` manage it directly; `vorlyn projects
...
Any Postgres ≥14 works. The app connects as the `vorlyn_login` role (migration `0024`) —
...
Setting `APP_LOGIN_PASSWORD` makes this same command also set `vorlyn_login`'s password —
...
| ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DATABASE_URL`                                                                      | Connects as `vorlyn_login` (above)                                                                                                                                                                                                           |
| `SESSION_SECRET`                                                                    | Random, 32+ chars                                                                                                                                                                                                                            |
| `API_BASE_URL` / `WEB_APP_URL` / `WEB_ORIGIN`                                       | Your instance's public URL(s)                                                                                                                                                                                                                |
| `PORT`                                                                              | The port this process listens on (default `3000`)                                                                                                                                                                                            |
| `VORLYN_AUTH_MODE`                                                                  | `single-user` (default — one fixed admin identity, no login screen) or `oidc` (real browser login against Keycloak, Authentik, Dex, Google, Okta, or any OIDC-compliant provider via discovery)                                              |
| `VORLYN_ADMIN_EMAIL` / `VORLYN_ADMIN_NAME`                                          | `single-user` mode only                                                                                                                                                                                                                      |
...
| `VORLYN_SERVE_WEB`                                                                  | `true` (default) serves the built SPA from this same process — set `false` if you're fronting it with your own reverse proxy or static host                                                                                                  |
| `VORLYN_ALLOW_LOCAL_BLOB_STORAGE`                                                   | `true` for this path — local disk under `NODE_ENV=production` is otherwise refused as a container-deployment safety default (`storageFromEnv`); a self-hosted instance with a persistent volume is exactly the legitimate case it exists for |
| `BLOB_STORAGE_DIR`                                                                  | Local-filesystem document storage (default, needs the flag above under `NODE_ENV=production`)                                                                                                                                                |
| `VORLYN_BLOBS_BUCKET` (+ `VORLYN_BLOBS_ENDPOINT` / `VORLYN_BLOBS_FORCE_PATH_STYLE`) | S3 or an S3-compatible backend (MinIO, R2, Backblaze B2) instead of local disk                                                                                                                                                               |
...
`pnpm build` here, not `pnpm --filter @vorlyn/api build` — `@vorlyn/api` has no `build` script of
its own; `pnpm typecheck`'s `tsc --build` is what produces its `dist/`. The `pnpm --filter @vorlyn/web
build` half matters just as much here as in the zero-install path above: `VORLYN_SERVE_WEB` defaults
...
vorlyn_...`) — no separate MCP-specific credential to configure. The `Dockerfile`'s `--target mcp`
