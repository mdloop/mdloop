...
export function setup() {
  ...
  check(r2, {
    'login sets session': (r) => String(r.headers['Set-Cookie'] || '').includes('mdloop_session'),
  });
  const cookies = jar.cookiesForURL(`${BASE}/`);
  const session = cookies.mdloop_session[0];
...
function auth(data) {
  const jar = http.cookieJar();
  jar.set(BASE, 'mdloop_session', data.session);
}
