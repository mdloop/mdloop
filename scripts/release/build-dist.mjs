#!/usr/bin/env node
/**
 * Builds `packages/vorlyn/dist` — the published npm package's payload.
 *
 ...
 * what gets inlined: every `@vorlyn/*` specifier is bundled, every other bare
 * specifier stays external. That is what makes the published npm surface
 * exactly one package — the internal `@vorlyn/*` names vanish into the
...
const outDir = path.join(vorlynPkgDir, 'dist');
...
if (!existsSync(webIndexHtml)) {
  fail(
    `no built web SPA at ${webIndexHtml}. Run "pnpm build" first — "pnpm typecheck" alone only ` +
      'emits .d.ts files for @vorlyn/web, never the actual "vite build" output.',
  );
}
...
//    not purely tsc output. `@vorlyn/web`'s tsconfig sets
...
const externalizeThirdParty = {
  ...
  setup(buildApi) {
    // Any bare specifier (not relative, not absolute) that isn't @vorlyn/*
    ...
    buildApi.onResolve({ filter: /^[^./]/ }, (args) => {
      if (args.path.startsWith('@vorlyn/')) return undefined;
...
cpSync(path.join(repoRoot, 'LICENSE'), path.join(vorlynPkgDir, 'LICENSE'));
cpSync(path.join(repoRoot, 'NOTICE'), path.join(vorlynPkgDir, 'NOTICE'));
cpSync(path.join(repoRoot, 'CHANGELOG.md'), path.join(vorlynPkgDir, 'CHANGELOG.md'));
const npmReadme = path.join(vorlynPkgDir, 'README.npm.md');
if (existsSync(npmReadme)) {
  cpSync(npmReadme, path.join(vorlynPkgDir, 'README.md'));
} else {
