#!/usr/bin/env node
/**
 * The packaging smoke test — proves the published `vorlyn` npm package
...
const STALE_IDENTIFIERS = ['chevron', 'baton']; // brand-check:allow — searched for, not used as branding
...
async function main() {
  ...
  for (const rel of [
    ...
    const p = path.join(vorlynPkgDir, rel);
    if (!existsSync(p)) {
      fail(
  ...
  step = 'npm pack';
  const packTmpDir = await mkdtempTracked('vorlyn-smoke-pack-');
  ...
  const installDir = await mkdtempTracked('vorlyn-smoke-install-');
  const projectDir = await mkdtempTracked('vorlyn-smoke-project-');
  const dataDir = await mkdtempTracked('vorlyn-smoke-data-');
  await writeFile(
  ...
  const bin = path.join(installDir, 'node_modules', '.bin', 'vorlyn');
  ...
  /**
   * SIGINT first (what `vorlyn open`'s own signal handler listens for —
   * `packages/cli/src/open.ts`), only escalating to SIGKILL if it doesn't
   * exit within the grace period. This isn't just politeness: `vorlyn
  ...
  async function stopOpenChildGracefully(assertCleanExit) {
    ...
    if (!graceful) {
      log('vorlyn open did not exit within 15s of SIGINT — escalating to SIGKILL.');
      openChild.kill('SIGKILL');
      await Promise.race([exitPromise, delay(5_000)]);
      if (assertCleanExit) fail('vorlyn open did not exit gracefully within 15s of SIGINT.');
      return;
    }
    const code = await exitPromise;
    if (assertCleanExit) assertEqual(code, 0, 'vorlyn open exit code after SIGINT');
  }
  cleanup.push(() => stopOpenChildGracefully(false));

  await waitFor(
    () => openOutput.out.includes('Vorlyn is running at'),
    30_000,
    'waiting for "Vorlyn is running at" from vorlyn open',
    () => `stdout so far:\n${openOutput.out}\nstderr so far:\n${openOutput.err}`,
  );
  ...
  const apiKey = await readVorlynApiKey(projectDir);
  const transport = new StreamableHTTPClientTransport(new URL(mcpEndpoint), {
    requestInit: { headers: { Authorization: `Bearer ${apiKey}` } },
  });
  const mcpClient = new Client({ name: 'vorlyn-smoke-test', version: '0.0.1' });
  ...
  if (!Array.isArray(docsList) || !docsList.some((d) => d.title === 'notes')) {
    fail(
      `list_documents didn't return the pushed "notes" doc — "vorlyn open" push may not have worked. Got: ${JSON.stringify(docsResult)}`,
    );
  }
  log(
  ...
  step = 'SIGINT shuts vorlyn open down cleanly';
  ...
  step = 'second phase: vorlyn serve start / status / stop';
  const serveDataDir = await mkdtempTracked('vorlyn-smoke-serve-data-');
  const serveEnv = {
    ...process.env,
    VORLYN_DATA_DIR: serveDataDir,
    PORT: '59931',
    MCP_PORT: '59932',
  };
...
async function isExecutableInTarball(tarballPath, entryName) {
  const extractDir = await mkdtempTracked('vorlyn-smoke-exec-check-');
 ...
 * packages/web/src/components/vorlyn-mark.tsx carries a legitimate,
...
async function assertNoBrandOrPathLeaks(tarballPath, entries) {
  ...
  const extractDir = await mkdtempTracked('vorlyn-smoke-extract-');
...
async function readVorlynApiKey(projectDir) {
  const credsPath = path.join(projectDir, '.vorlyn', 'credentials');
 ...
 * machine may genuinely have an unrelated, pre-existing `vorlyn serve`
