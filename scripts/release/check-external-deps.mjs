 ...
 * `packages/mdloop/dist/.metafile.json` (written by `build-dist.mjs`) and
 * asserts two things against the hand-written `packages/mdloop/package.json`:
 *
 ...
 *     `npx mdloop open` would crash at module-link time the moment that
...
const metafilePath = path.join(mdloopPkgDir, 'dist', '.metafile.json');
...
const mdloopPkg = JSON.parse(readFileSync(path.join(mdloopPkgDir, 'package.json'), 'utf8'));
...
const declaredDeps = mdloopPkg.dependencies ?? {};
const declaredOptionalDeps = mdloopPkg.optionalDependencies ?? {};
...
for (const pkg of usedPackages) {
  if (!declaredPackages.has(pkg)) {
    errors.push(
      `"${pkg}" is imported by the bundled output but not declared in packages/mdloop/package.json's ` +
...
for (const pkg of declaredPackages) {
  if (!usedPackages.has(pkg)) {
    errors.push(
      `"${pkg}" is declared in packages/mdloop/package.json but no bundle actually imports it — ` +
...
for (const pkgName of usedPackages) {
  ...
  if (sourceRange === undefined) {
    errors.push(
      `"${pkgName}" is declared in packages/mdloop/package.json (range "${declaredRange}") but no ` +
        `packages/*/package.json in this workspace lists it as a real dependency to check that ` +
        `range against.`,
    );
  } else if (sourceRange !== declaredRange) {
    errors.push(
      `"${pkgName}" version range mismatch: packages/mdloop/package.json says "${declaredRange}", ` +
  ...
  if (kinds?.has('import-statement')) {
    errors.push(
      `"${pkgName}" is in optionalDependencies but is statically imported ("import-statement") in ` +
        `at least one bundle — this would crash "npx mdloop open" at boot the moment it's omitted ` +
