 ...
 * published `mdloop` npm version, combined with the real `mdloop-sync`
...
async function main() {
  ...
  step = 'preflight: mdloop marketplace registered';
  const { stdout: marketplaces } = await execFileAsync(claudeBin, [
    'plugin',
    'marketplace',
    'list',
  ]);
  if (!/mdloop/.test(marketplaces)) {
    fail(
      'no "mdloop" marketplace configured (claude plugin marketplace list). Add one first — ' +
        '`claude plugin marketplace add imjasdeepk/mdloop` for the real distribution path — ' +
  ...
  step = `npm install mdloop@${target} into an isolated prefix`;
  const npmPrefix = await mkdtempTracked('mdloop-smoke-plugin-npm-');
  execFileSync(
    'npm',
    ['install', '-g', `mdloop@${target}`, '--prefix', npmPrefix, '--no-audit', '--no-fund'],
    {
      stdio: 'inherit',
    },
  );
  const mdloopBin = path.join(npmPrefix, 'bin', 'mdloop');
  const versionResult = await execFileAsync(mdloopBin, ['--help']);
  assertContains(versionResult.stdout, 'Usage:', 'mdloop --help stdout');
  log(`installed from the real registry at tag/version "${target}", isolated at ${npmPrefix}`);

  step = 'set up an isolated git project';
  const projectDir = await mkdtempTracked('mdloop-smoke-plugin-project-');
  const dataDir = await mkdtempTracked('mdloop-smoke-plugin-data-');
  ...
  step = 'run the real SessionStart hook (mdloop-ensure.sh)';
  const hookPath = await resolveHookScript(claudeBin);
  const port = 59401;
  const mcpPort = 59402;
  const hookEnv = {
    ...process.env,
    MDLOOP_CLI_PATH: mdloopBin,
    MDLOOP_DATA_DIR: dataDir,
    PORT: String(port),
    MCP_PORT: String(mcpPort),
    // Isolated bin dir FIRST, not a wiped PATH — the hook's own `command -v
    // mdloop` must resolve to this isolated install, not this machine's
    ...
    PATH: `${path.dirname(mdloopBin)}:${process.env.PATH ?? ''}`,
  };
  cleanup.push(() => stopServeGracefully(mdloopBin, dataDir));
  ...
  const manifestPath = path.join(projectDir, '.mdloop', 'manifest.json');
  ...
  if (!new RegExp(`mdloop:.*${mcpPort}`).test(mcpListBefore.stdout)) {
    fail(
      `"claude mcp list" (run from the test project dir) doesn't show a mdloop entry on port ${mcpPort}:\n${mcpListBefore.stdout}`,
    );
  }
  ...
  step = "push the doc (mdloop-sync skill's own command)";
  await execFileAsync(mdloopBin, ['push', projectDir, '--note', 'Initial design note'], {
    env: hookEnv,
  });
  ...
  if (!documentId)
    fail('mdloop push did not record a documentId for design-note.md in the manifest.');
  log(`pushed — document ${documentId}`);

  step = 'connect over real MCP (same client the mdloop-review skill uses)';
  const apiKey = JSON.parse(
    await readFile(path.join(projectDir, '.mdloop', 'credentials'), 'utf8'),
  ).apiKey;
  const transport = new StreamableHTTPClientTransport(new URL(`http://127.0.0.1:${mcpPort}/mcp`), {
    requestInit: { headers: { Authorization: `Bearer ${apiKey}` } },
  });
  const mcp = new Client({ name: 'mdloop-smoke-plugin-loop', version: '0.0.1' });
  ...
  log(`\nsmoke-published-plugin: ALL GREEN in ${elapsed}s against mdloop@${target}.`);
}
 ...
 * <version>/hooks/mdloop-ensure.sh` — a versioned cache of what got
 * installed, not this working tree. Resolving it from there (rather than
 * hardcoding `claude-plugin/hooks/mdloop-ensure.sh` relative to this
...
async function resolveHookScript(claudeBin) {
  ...
  const versionMatch = listOutput.match(/mdloop-sync@mdloop\s*\n\s*Version:\s*(\S+)/);
  if (!versionMatch) {
    fail(
      `could not find "mdloop-sync@mdloop"'s installed version from "claude plugin list". Output:\n${listOutput}`,
    );
  }
...
async function removeMcpRegistration(claudeBin, projectDir) {
  await execFileAsync(claudeBin, ['mcp', 'remove', 'mdloop', '--scope', 'local'], {
    cwd: projectDir,
  }).catch(() => undefined);
}

async function stopServeGracefully(mdloopBin, dataDir) {
  await execFileAsync(mdloopBin, ['serve', 'stop'], {
    env: { ...process.env, MDLOOP_DATA_DIR: dataDir },
    timeout: 20_000,
  }).catch(() => undefined);
}
