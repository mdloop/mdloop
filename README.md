<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="docs/assets/mdloop-mark-dark.svg">
    <img src="docs/assets/mdloop-mark-light.svg" alt="" width="76" height="76">
  </picture>
</p>
<h1 align="center">mdloop</h1>
...
> `0.3.0` on npm covers the local, single-user path below. Team/self-hosting still means building
...
mdloop is that missing loop: the agent publishes, a human reviews and comments right on the doc,
the agent reads the feedback back and revises. Repeat until it's signed off.

![The mdloop review loop: an agent publishes a doc, a human comments and suggests an edit, the agent reads the feedback back](docs/assets/review-loop.gif)
...
- **SSO against your existing directory** — `MDLOOP_AUTH_MODE=oidc` does real browser login against
...
registers the MCP connection with Claude Code itself, so the tools are just there. Nothing below
this is needed for that case.

**A shared team instance, Cursor, Codex CLI, or any other MCP client** needs two things first:

- **Endpoint** — `mdloop serve status` prints it; default `http://127.0.0.1:58744/mcp`.
- **Key** — `.mdloop/credentials` in a linked folder (`{"apiKey": "mdloop_..."}`), or mint one in
  the web app. Sent as `Authorization: Bearer mdloop_...`.

**Claude Code, against a shared team instance** (its MCP URL isn't something the plugin can
discover on its own):

```sh
claude mcp add --transport http mdloop http://127.0.0.1:58744/mcp \
  --header "Authorization: Bearer mdloop_..."
```
...
```json
{
  "mcpServers": {
    "mdloop": {
      ...
      "headers": { "Authorization": "Bearer mdloop_..." }
    }
  }
}
```
...
[mcp_servers.mdloop]
url = "http://127.0.0.1:58744/mcp"
bearer_token_env_var = "MDLOOP_API_KEY"
```

**Anything else** — any MCP client speaking streamable HTTP: same URL, same header.

Then just ask it — _"push this spec to mdloop for review"_ … _"what did the reviewer say?"_ The
...
call a hosted or distributed derivative "mdloop" — see NOTICE.
