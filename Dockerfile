...
# mdloop — production container image. One Dockerfile, one shared deps ->
...
#   docker buildx build --platform linux/arm64 --target api     -t mdloop-api:<digest-tag>     .
#   docker buildx build --platform linux/arm64 --target mcp     -t mdloop-mcp:<digest-tag>     .
#   docker buildx build --platform linux/arm64 --target jobs    -t mdloop-jobs:<digest-tag>    .
#   docker buildx build --platform linux/arm64 --target migrate -t mdloop-migrate:<digest-tag> .
...
# owner — never mdloop_login; see packages/persistence/src/migrate-main.ts):
#   docker run --rm -e DATABASE_URL=... -e DB_SSL_CA_PATH=/app/rds-ca-bundle.pem mdloop-migrate:<digest-tag>
# which is exactly:
...
# output — @mdloop/persistence, and migrate-main.js with it, is already a
# transitive prod dependency of @mdloop/api, so no fourth `pnpm deploy` is
...
# the target dir (verified locally: node_modules/@mdloop/* are real compiled
# output, not symlinks back into the workspace) — it's the correct flag here,
# not a fallback.
RUN pnpm --filter @mdloop/api deploy --prod --legacy /deploy/api \
 && pnpm --filter @mdloop/mcp deploy --prod --legacy /deploy/mcp \
 && pnpm --filter @mdloop/jobs deploy --prod --legacy /deploy/jobs
...
RUN for svc in api mcp jobs; do \
      ...
      for dep in /deploy/$svc/node_modules/@mdloop/*; do \
...
# a fourth `pnpm deploy`, since @mdloop/persistence (migrate-main.js included)
# is already a transitive prod dependency of @mdloop/api.
# -----------------------------------------------------------------------------
FROM runtime-base AS migrate
COPY --from=build --chown=10001:10001 /deploy/api ./
USER 10001:10001
CMD ["node", "node_modules/@mdloop/persistence/dist/migrate-main.js"]
