 ...
 * "@vorlyn/<name>" specifier. Rules match both spellings.
 */
const pkgs = (names) => `^packages/(${names})/|(^|/)@vorlyn/(${names})$`;
  ...
  forbidden: [
    ...
    },
    {
      ...
      comment:
        ...
        '"vorlyn open" (Phase E) is the one deliberate exception to "no server-side imports": it ' +
        'directly drives an embedded Postgres (startEmbeddedPostgres/migrate, @vorlyn/persistence) ' +
        'to set up the local instance it then spawns as separate OS processes — it never imports ' +
        'app/domain/api/mcp code, only persistence. @vorlyn/api is a real dependency too, but only ' +
